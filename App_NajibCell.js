...
const THEMES = {
  ...
  navy: {
    name: "Midnight Navy", icon: "🌙",
    bg: "#0d1117", surface: "#161b22", card: "#1c2128", border: "#30363d",
    accent: "#58a6ff", accentDim: "#58a6ff22", green: "#3fb950",
    red: "#f85149", yellow: "#d29922", text: "#c9d1d9", muted: "#8b949e",
  },
  slate: {
    name: "Slate Pro", icon: "🪨",
    bg: "#0f1117", surface: "#171923", card: "#1e2132", border: "#2d3350",
    accent: "#94a3b8", accentDim: "#94a3b822", green: "#4ade80",
    red: "#f87171", yellow: "#fbbf24", text: "#e2e8f0", muted: "#64748b",
  },
  forest: {
    name: "Forest Dark", icon: "🌿",
    bg: "#0d1210", surface: "#141c18", card: "#1a2420", border: "#2a3830",
    accent: "#6ee7b7", accentDim: "#6ee7b722", green: "#34d399",
    red: "#f87171", yellow: "#fbbf24", text: "#d1fae5", muted: "#6b8a78",
  },
  espresso: {
    name: "Espresso", icon: "☕",
    bg: "#12100e", surface: "#1c1815", card: "#26211c", border: "#3d3530",
    accent: "#d4956a", accentDim: "#d4956a22", green: "#7ec98a",
    red: "#e06c75", yellow: "#e5c07b", text: "#ede0d4", muted: "#8a7a70",
  },
  obsidian: {
    name: "Pure Black", icon: "🌑",
    bg: "#000000", surface: "#0a0a0a", card: "#111111", border: "#222222",
    accent: "#ffffff", accentDim: "#ffffff15", green: "#00ff88",
    red: "#ff3b3b", yellow: "#ffcc00", text: "#ffffff", muted: "#555555",
  },
  charcoal: {
    name: "Charcoal", icon: "🌫️",
    bg: "#1a1a1a", surface: "#222222", card: "#2a2a2a", border: "#3a3a3a",
    accent: "#a0a0a0", accentDim: "#a0a0a022", green: "#6dbf85",
    red: "#e05c5c", yellow: "#d4a85a", text: "#e0e0e0", muted: "#707070",
  },
  white: {
    name: "Clean White", icon: "☀️",
    bg: "#f8f9fa", surface: "#ffffff", card: "#ffffff", border: "#e2e8f0",
    accent: "#2563eb", accentDim: "#2563eb18", green: "#16a34a",
    red: "#dc2626", yellow: "#d97706", text: "#111827", muted: "#6b7280",
  },
  light: {
    name: "Light Gray", icon: "🌤️",
    bg: "#f1f5f9", surface: "#f8fafc", card: "#ffffff", border: "#cbd5e1",
    accent: "#7c3aed", accentDim: "#7c3aed18", green: "#059669",
    red: "#e11d48", yellow: "#d97706", text: "#1e293b", muted: "#64748b",
  },
  paper: {
    name: "Paper", icon: "📄",
    bg: "#faf8f5", surface: "#ffffff", card: "#ffffff", border: "#e8e0d5",
    accent: "#b45309", accentDim: "#b4530918", green: "#15803d",
    red: "#b91c1c", yellow: "#92400e", text: "#1c1917", muted: "#78716c",
  },
};

const getTheme = () => {
  const saved = localStorage.getItem("BSM_theme") || "cyan";
...
const ThemeSwitcher = ({ onThemeChange }) => {
  ...
  const [current, setCurrent] = useState(localStorage.getItem("BSM_theme") || "cyan");
  const select = (key) => {
    localStorage.setItem("BSM_theme", key);
    setCurrent(key);
    setOpen(false);
    onThemeChange();
  };
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          <div style={{ fontSize: 32, fontWeight: 800, marginBottom: 8 }}>
            <span style={{ color: T.accent }}>📱</span> Najib Cell
          </div>
          <div style={{ color: T.muted, fontSize: 13, letterSpacing: 2, textTransform: "uppercase" }}>Cell Phone Management</div>
        </div>
...
function PrintInvoice({ inv, locations, onClose }) {
  ...
  const print = () => {
    const html = `<!DOCTYPE html><html><head><title>Najib Cell — Invoice ${inv.id}</title><style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      <div class="footer">Thank you for your business! • Najib Cell Management</div>
    ...
    <div style={{ position: "fixed", inset: 0, background: "#000a", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center" }} className="no-print">
      <div style={{ background: "white", color: "#000", borderRadius: 12, padding: 40, width: "100%", maxWidth: 600, maxHeight: "90vh", overflow: "auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <h2 style={{ fontSize: 22, fontWeight: 800 }}>📱 Najib Cell</h2>
        ...
        <div style={{ marginTop: 40, paddingTop: 16, borderTop: "1px solid #eee", textAlign: "center", fontSize: 11, color: "#999" }}>
          Thank you for your business! • Najib Cell Management
        </div>
      </div>
    </div>
  );
...
function Dashboard({ invoices, products, locations, userProfile, setPage }) {
  ...
  const LOW_STOCK = 5;
  const lowStockItems = products.filter(p => (p.totalStock || 0) > 0 && (p.totalStock || 0) <= LOW_STOCK);
  ...
  return (
    <div className="page">
      ...
      {lowStockItems.length > 0 && (
        <div onClick={() => setPage("inventory")} style={{ background:"#7f1d1d22", border:"1px solid #ef444466", borderRadius:10, padding:"10px 16px", marginBottom:16, cursor:"pointer", display:"flex", alignItems:"center", gap:10 }}>
          <span style={{ fontSize:18 }}>⚠️</span>
          <div>
            <div style={{ fontSize:13, fontWeight:700, color:"#ef4444" }}>Low Stock Alert — {lowStockItems.length} product{lowStockItems.length>1?"s":""} running low</div>
            <div style={{ fontSize:12, color:T.muted }}>{lowStockItems.slice(0,3).map(p=>p.name).join(", ")}{lowStockItems.length>3?` +${lowStockItems.length-3} more`:""} · Click to view inventory</div>
          </div>
        </div>
      )}
...
function Invoices({ invoices, setInvoices, products, locations, clients, suppliers, onRefresh, userProfile }) {
  ...
  const [newInv, setNewInv] = useState({ type: "sell", location_id: "", customer: "", client_id: "", supplier_id: "", date: new Date().toISOString().slice(0, 10), items: [], discountType: "fixed", discountValue: 0, shipmentType: "fixed", shipmentValue: 0, distributeShipment: false, shipmentCompany: "", shipmentPaymentStatus: "pending", shipmentPaymentMethod: "cash_usd", bsmProfit: "", paymentStatus: "paid", amountPaid: "", paymentMethod: "cash_usd", paymentReference: "" });
  ...
  const saveInvoice = async () => {
    ...
    // Calculate split amounts for purchase invoices
    const bsmAmount = newInv.type === "buy" ? (+newInv.bsmProfit || 0) : 0;
    // supplier gets: grand total - shipping - BSM profit
    const supplierAmount = newInv.type === "buy" ? grandTotal - shipmentAmt - bsmAmount : grandTotal;
    ...
    const invoiceData = {
      ...
      supplier_amount: newInv.type === "buy" ? supplierAmount : null,
      partial_bsm: newInv.type === "buy" && bsmAmount > 0 ? bsmAmount : null,
    };
  ...
  const saveEdit = async () => {
    ...
    const amountPaid = editInv.payment_status === "paid"
      ...
      : editInv.payment_status === "partial"
        ? Math.min(+editInv.amount_paid || 0, newTotal)
        : 0;
          ...
          <div style={{ background:T.card, border:`1px solid ${T.accent}44`, borderRadius:16, padding:28, width:"100%", maxWidth:820, maxHeight:"92vh", overflowY:"auto" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:16 }}>
              <div style={{ background:T.surface, borderRadius:10, padding:14 }}>
                <div style={{ fontSize:11, color:T.muted, marginBottom:8, textTransform:"uppercase" }}>🏷️ Discount</div>
                <div style={{ display:"flex", gap:8 }}>
                  <select value={editInv.discount_type||"fixed"} onChange={e => setEditInv({...editInv,discount_type:e.target.value})} style={{ width:"auto" }}>
                    <option value="fixed">Fixed ($)</option>
                    <option value="pct">Percent (%)</option>
                  </select>
                  <input type="number" value={editInv.discount_value||0} min="0" onChange={e => setEditInv({...editInv,discount_value:e.target.value})} style={{ width:90 }} />
                </div>
              </div>
              <div style={{ background:T.surface, borderRadius:10, padding:14 }}>
            ...
            {(() => {
              ...
              return (
                <div style={{ background:T.surface, borderRadius:10, padding:14, marginBottom:16, fontFamily:T.mono }}>
                  ...
                  {editInv.type === "buy" && (ship > 0 || (+editInv.partial_bsm||0) > 0) && (
                    <div style={{ marginTop:8, paddingTop:8, borderTop:`1px solid ${T.border}33` }}>
                      {(+editInv.partial_bsm||0) > 0 && <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:2 }}><span style={{ color:T.accent }}>🏦 BSM Profit</span><span style={{ color:T.accent }}>{fmt(+editInv.partial_bsm||0)}</span></div>}
                      {ship > 0 && <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:2 }}><span style={{ color:T.yellow }}>🚢 Shipping</span><span style={{ color:T.yellow }}>{fmt(ship)}</span></div>}
                      <div style={{ display:"flex", justifyContent:"space-between", fontSize:13, fontWeight:700, color:T.green, marginTop:4 }}><span>🏭 Supplier Gets</span><span>{fmt(Math.max(0, tot - ship - (+editInv.partial_bsm||0)))}</span></div>
                    </div>
                  )}
                </div>
              );
        ...
        <div style={{ background: T.card, border: `1px solid ${T.accent}44`, borderRadius: 12, padding: 24, marginBottom: 24 }}>
          ...
          {newInv.items.length > 0 && (
            <div style={{ background: T.accentDim, borderRadius: 10, padding: 16, marginBottom: 16, fontFamily: T.mono }}>
              ...
              {newInv.type === "buy" && (shipmentAmt > 0 || (+newInv.bsmProfit||0) > 0) && (
                <div style={{ marginTop:8, paddingTop:8, borderTop:`1px solid ${T.border}33` }}>
                  {(+newInv.bsmProfit||0) > 0 && <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:2 }}><span style={{ color:T.accent }}>🏦 BSM Profit</span><span style={{ color:T.accent }}>{fmt(+newInv.bsmProfit)}</span></div>}
                  {shipmentAmt > 0 && <div style={{ display:"flex", justifyContent:"space-between", fontSize:12, marginBottom:2 }}><span style={{ color:T.yellow }}>🚢 Shipping Co.</span><span style={{ color:T.yellow }}>{fmt(shipmentAmt)}</span></div>}
                  <div style={{ display:"flex", justifyContent:"space-between", fontSize:14, fontWeight:800, color:T.green, marginTop:4 }}><span>🏭 Supplier Gets</span><span>{fmt(Math.max(0, grandTotal - shipmentAmt - (+newInv.bsmProfit||0)))}</span></div>
                </div>
              )}
            </div>
          )}
          ...
          {newInv.items.length > 0 && (
            <div style={{ background: T.surface, borderRadius: 10, padding: 16, marginBottom: 16, border: `1px solid ${T.yellow}33` }}>
              ...
              {/* BSM Profit field — purchase invoices only */}
              {newInv.type === "buy" && (
                <div style={{ background: T.surface, borderRadius: 10, padding: 16, marginBottom: 16, border: `1px solid ${T.accent}33` }}>
                  <div style={{ fontSize: 12, color: T.accent, fontWeight: 700, marginBottom: 10, textTransform: "uppercase", letterSpacing: 1 }}>🏦 BSM Profit (included in product prices)</div>
                  <div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                    <div style={{ flex: 1, minWidth: 140 }}>
                      <div style={{ fontSize: 11, color: T.muted, marginBottom: 4 }}>Total BSM Profit ($)</div>
                      <input type="number" value={newInv.bsmProfit} onChange={e => setNewInv({...newInv, bsmProfit: e.target.value})} placeholder="0.00" min="0" />
                    </div>
                    {newInv.bsmProfit > 0 && (
                      <div style={{ background: T.card, borderRadius: 8, padding: "10px 16px", fontSize: 13, fontFamily: T.mono }}>
                        <div style={{ color: T.muted, fontSize: 11, marginBottom: 4 }}>Supplier gets:</div>
                        <div style={{ color: T.green, fontWeight: 800, fontSize: 16 }}>{fmt(Math.max(0, subtotal - totalDiscount - (+newInv.bsmProfit||0)))}</div>
                        <div style={{ color: T.accent, fontSize: 11, marginTop: 2 }}>BSM keeps: {fmt(+newInv.bsmProfit||0)}</div>
                      </div>
                    )}
                  </div>
                </div>
  ...
  const { data: items } = await supabase.from("invoice_items").select("*").eq("invoice_id", inv.id);
  setEditInv({ ...inv, payment_status: inv.payment_status || "pending", amount_paid: inv.amount_paid || 0, payment_method: inv.payment_method || "cash_usd", editItems: items || [] });
...
function SalesOrders({ products, locations, invoices, setInvoices, onRefresh, clients, suppliers }) {
  ...
  const [convertForm, setConvertForm] = useState({ customerName: "", freightPct: "", freightSupplierId: "" });
  ...
  const saveOrder = async () => {
    ...
    const { error } = await supabase.from("sales_orders").insert({
      ...
      customer: newOrder.customer, status: "draft", notes: newOrder.notes || null,
      ...
      reserve_stock: newOrder.reserveStock || false,
      freight_pct: +newOrder.freightPct || 0,
      freight_supplier_id: newOrder.freightSupplierId ? +newOrder.freightSupplierId : null,
    });
    if (!error) {
  ...
  const openConvertModal = (order) => {
    setConvertModal(order);
    setConvertForm({
      customerName: order.customer,
      freightPct: order.freight_pct > 0 ? order.freight_pct : "",
      freightSupplierId: order.freight_supplier_id || "",
    });
    const qtys = {};
    order.items.forEach((item, i) => { qtys[i] = item.quantity; });
    setPartialQtys(qtys);
  };

  const convertToInvoice = async () => {
    if (!convertModal) return;
    const order = convertModal;
    setConverting(order.id);
    const today = new Date().toISOString().slice(0,10);
    const deliveringItems = order.items.map((item, i) => ({ ...item, deliverQty: Math.min(+partialQtys[i]||0, +item.quantity) })).filter(item => item.deliverQty > 0);
    const remainingItems  = order.items.map((item, i) => ({ ...item, remainQty: +item.quantity - Math.min(+partialQtys[i]||0, +item.quantity) })).filter(item => item.remainQty > 0);
    if (deliveringItems.length === 0) { alert("Enter at least 1 item to deliver."); setConverting(null); return; }
    const invId = "INV-" + Math.random().toString(36).slice(2, 8).toUpperCase();
    const type = order.type || "sell";
    const subtotal = deliveringItems.reduce((s, i) => s + i.deliverQty * i.price, 0);
    const freightAmt = convertForm.freightPct > 0 ? subtotal * (+convertForm.freightPct / 100) : 0;
    const total = subtotal + freightAmt;
    const cogs = deliveringItems.reduce((s, i) => s + i.deliverQty * (i.cost||0), 0);
    let clientId = order.client_id;
    if (convertForm.customerName !== order.customer) {
      const found = clients.find(c => c.name.toLowerCase() === convertForm.customerName.toLowerCase());
      if (found) clientId = found.id;
    }
    const { error } = await supabase.from("invoices").insert({
      id: invId, type, date: today, location_id: order.location_id,
      customer: convertForm.customerName || order.customer,
      status: "pending", payment_status: "pending",
      total, cogs, profit: total - cogs,
      client_id: clientId || null, supplier_id: order.supplier_id || null,
      discount_type: "fixed", discount_value: 0, shipment_type: "fixed", shipment_value: 0,
    });
    if (error) { alert("Error: " + error.message); setConverting(null); return; }
    await supabase.from("invoice_items").insert(
      deliveringItems.map(i => ({ invoice_id: invId, product_id: +i.product_id, product_name: i.product_name, quantity: i.deliverQty, price: i.price, original_price: i.price, discount_pct: 0, cost: i.cost||0 }))
    );
    if (freightAmt > 0 && convertForm.freightSupplierId) {
      await supabase.from("supplier_payments").insert({ supplier_id: +convertForm.freightSupplierId, date: today, amount: freightAmt, type: "charge", notes: `Freight ${convertForm.freightPct}% for order ${order.id} → Invoice ${invId}` });
    }
    if (!order.reserve_stock) {
      for (const item of deliveringItems) {
        ...
        if (stockRow) await supabase.from("stock").update({ quantity: Math.max(0, stockRow.quantity - item.deliverQty) }).eq("id", stockRow.id);
      }
    }
    if (remainingItems.length === 0) {
      await supabase.from("sales_orders").update({ status: "invoiced" }).eq("id", order.id);
    } else {
      await supabase.from("sales_orders").update({ status: "partial" }).eq("id", order.id);
      await supabase.from("sales_order_items").delete().eq("order_id", order.id);
      await supabase.from("sales_order_items").insert(
        remainingItems.map(i => ({ order_id: order.id, product_id: +i.product_id, product_name: i.product_name, quantity: i.remainQty, price: i.price, cost: i.cost||0 }))
      );
    }
    setConvertModal(null); setConverting(null); loadOrders(); onRefresh();
  };
  ...
  const printOrder = (order) => {
    ...
    const freightPct = +(order.freight_pct || 0);
    const freightAmt = freightPct > 0 ? subtotal * (freightPct/100) : 0;
    const grandTotal = subtotal + freightAmt;
    const freightSupplier = order.freight_supplier_id ? suppliers.find(s=>+s.id===+order.freight_supplier_id) : null;
    ...
    const html = `<!DOCTYPE html><html><head><title>Najib Cell — Order ${order.id}</title>
    <style>
      *{box-sizing:border-box;margin:0;padding:0;}
      body{font-family:Arial,sans-serif;padding:40px;font-size:13px;color:#000;}
      .header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:30px;}
      .company{font-size:22px;font-weight:800;color:#000;}
      .subtitle{font-size:13px;color:#666;margin-top:4px;}
      .order-id{font-size:18px;font-weight:800;text-align:right;}
      .status{display:inline-block;padding:4px 12px;border-radius:20px;font-size:11px;font-weight:700;text-transform:uppercase;margin-top:6px;}
      .info-grid{display:grid;grid-template-columns:1fr 1fr;gap:20px;margin-bottom:24px;padding:16px;background:#f8f9fa;border-radius:8px;}
      .info-label{font-size:11px;color:#666;text-transform:uppercase;letter-spacing:1px;margin-bottom:3px;}
      .info-value{font-size:14px;font-weight:700;}
      table{width:100%;border-collapse:collapse;margin-bottom:20px;}
      th{background:#000;color:#fff;padding:10px;text-align:left;font-size:12px;}
      th:nth-child(2),th:nth-child(3),th:nth-child(4){text-align:right;}
      td{padding:9px 10px;border-bottom:1px solid #eee;font-size:13px;}
      .total-row td{font-weight:800;background:#f0f0f0;font-size:14px;}
      .freight-row td{color:#d97706;background:#fffbeb;}
      .grand-row td{font-weight:800;background:#000;color:#fff;font-size:15px;}
      .footer{margin-top:30px;text-align:center;font-size:11px;color:#999;border-top:1px solid #eee;padding-top:16px;}
    </style></head>
    <body>
      <div class="header">
        <div>
          <div class="company">📱 Najib Cell</div>
          <div class="subtitle">Sales Order</div>
        </div>
        <div style="text-align:right">
          <div class="order-id">${order.id}</div>
          <div class="status" style="background:${order.status==="confirmed"?"#fef3c7;color:#92400e":order.status==="invoiced"?"#d1fae5;color:#065f46":"#fee2e2;color:#991b1b"}">${order.status?.toUpperCase()}</div>
        </div>
      </div>
      <div class="info-grid">
        <div><div class="info-label">Client</div><div class="info-value">${order.customer}</div></div>
        <div><div class="info-label">Date</div><div class="info-value">${order.date}</div></div>
        <div><div class="info-label">Location</div><div class="info-value">${order.locationName||""}</div></div>
        <div><div class="info-label">Total Qty</div><div class="info-value">${totalQty} pcs</div></div>
        ${order.notes ? `<div style="grid-column:1/-1"><div class="info-label">Notes</div><div class="info-value">${order.notes}</div></div>` : ""}
      </div>
      <table>
        ...
        <tbody>
          ...
          <tr class="freight-row">
            <td>🚚 Freight ${freightPct}%${freightSupplier?" — "+freightSupplier.name:""}</td>
            <td></td><td></td>
            <td style="text-align:right">+$${freightAmt.toFixed(2)}</td>
          </tr>
          <tr class="grand-row">
            <td>GRAND TOTAL</td>
            <td style="text-align:center">${totalQty} pcs</td>
      ...
      <div class="footer">Printed: ${new Date().toLocaleDateString()} • Najib Cell Management</div>
      <script>window.onload=function(){window.print();window.onafterprint=function(){window.close();}}</script>
    </body></html>`;
  ...
  const statusColor = { draft: T.muted, confirmed: T.yellow, partial: T.accent, invoiced: T.green, cancelled: T.red };
    ...
    <div className="page">
      ...
      {showCreate && (
        <div style={{ background: T.card, border: `1px solid ${T.accent}44`, borderRadius: 12, padding: 24, marginBottom: 24 }}>
          ...
          {/* Freight Fee */}
          <div style={{ background: T.surface, borderRadius: 10, padding: 14, marginBottom: 16, border: `1px solid ${newOrder.freightPct > 0 ? T.yellow+"66" : T.border}` }}>
            <div style={{ fontSize: 12, color: T.yellow, fontWeight: 700, marginBottom: 10 }}>🚚 Freight Fee</div>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
              <div style={{ flex: 1, minWidth: 160 }}>
                <div style={{ fontSize: 11, color: T.muted, marginBottom: 4 }}>FRT SUPPLIER</div>
                <select value={newOrder.freightSupplierId} onChange={e => setNewOrder({...newOrder, freightSupplierId: e.target.value})} style={{ width: "100%" }}>
                  <option value="">None</option>
                  {suppliers.filter(s => s.name.toUpperCase().includes("FRT")).map(s => (
                    <option key={s.id} value={s.id}>{s.name}</option>
                  ))}
                </select>
              </div>
              <div style={{ width: 120 }}>
                <div style={{ fontSize: 11, color: T.muted, marginBottom: 4 }}>FREIGHT %</div>
                <input type="number" value={newOrder.freightPct} onChange={e => setNewOrder({...newOrder, freightPct: e.target.value})} placeholder="0" min="0" max="100" />
              </div>
              {newOrder.freightPct > 0 && (
                <div style={{ background: T.card, borderRadius: 8, padding: "8px 14px", fontFamily: T.mono, fontSize: 13 }}>
                  <span style={{ color: T.muted }}>Freight: </span>
                  <span style={{ color: T.yellow, fontWeight: 700 }}>
                    {fmt(newOrder.items.reduce((s,i)=>s+i.qty*i.price,0) * (+newOrder.freightPct/100))}
                  </span>
                </div>
              )}
            </div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <Btn outline onClick={() => { setShowCreate(false); setNewOrder({ type:"sell", location_id:"", customer:"", client_id:"", supplier_id:"", date:new Date().toISOString().slice(0,10), notes:"", items:[], reserveStock:false, freightPct:"", freightSupplierId:"" }); }}>Cancel</Btn>
          </div>
        </div>
      )}
      ...
      {convertModal && (
        <div style={{ position:"fixed", inset:0, background:"#000a", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }}>
          <div style={{ background:T.card, border:`1px solid ${T.green}44`, borderRadius:16, padding:28, width:"100%", maxWidth:480 }}>
            <h3 style={{ fontSize:16, fontWeight:700, color:T.green, marginBottom:6 }}>⚡ Partial / Full Delivery</h3>
            <p style={{ fontSize:12, color:T.muted, marginBottom:12, fontFamily:T.mono }}>{convertModal.id} — enter qty to deliver now</p>
            <div style={{ background:T.surface, borderRadius:10, padding:12, marginBottom:16 }}>
              {convertModal.items.map((item, i) => { const dq = partialQtys[i]!==undefined?partialQtys[i]:item.quantity; const rem = +item.quantity - Math.min(+dq,+item.quantity); return (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:10, marginBottom:8, padding:"8px 10px", background:T.card, borderRadius:8, border:`1px solid ${T.border}` }}>
                  <div style={{ flex:1 }}><div style={{ fontSize:13, fontWeight:600 }}>{item.product_name}</div><div style={{ fontSize:11, color:T.muted }}>Ordered: {item.quantity} · {fmt(item.price)} each</div></div>
                  <div style={{ textAlign:"center" }}><div style={{ fontSize:10, color:T.muted, marginBottom:2 }}>DELIVER</div><input type="number" min="0" max={item.quantity} value={dq} onChange={e=>setPartialQtys({...partialQtys,[i]:Math.min(+e.target.value,+item.quantity)})} style={{ width:70, textAlign:"center", fontWeight:700 }} /></div>
                  <div style={{ textAlign:"center" }}><div style={{ fontSize:10, color:T.muted, marginBottom:2 }}>REMAINING</div><div style={{ width:60, textAlign:"center", fontFamily:T.mono, fontSize:13, color:rem>0?T.yellow:T.muted, padding:"6px 0" }}>{rem}</div></div>
                </div>
              ); })}
              {(() => { const td=convertModal.items.reduce((s,item,i)=>s+Math.min(+(partialQtys[i]||0),+item.quantity),0); const tr2=convertModal.items.reduce((s,item,i)=>s+(+item.quantity-Math.min(+(partialQtys[i]||0),+item.quantity)),0); const ia=convertModal.items.reduce((s,item,i)=>s+Math.min(+(partialQtys[i]||0),+item.quantity)*item.price,0); return (
                <div style={{ display:"flex", gap:16, marginTop:10, padding:"8px 10px", background:T.green+"11", borderRadius:8, border:`1px solid ${T.green}33` }}>
                  <div><div style={{ fontSize:10, color:T.muted }}>DELIVERING NOW</div><div style={{ fontWeight:700, color:T.green }}>{td} pcs · {fmt(ia)}</div></div>
                  {tr2>0&&<div><div style={{ fontSize:10, color:T.muted }}>REMAINING</div><div style={{ fontWeight:700, color:T.yellow }}>{tr2} pcs</div></div>}
                  {tr2===0&&<div style={{ fontSize:12, color:T.green }}>✅ Full delivery</div>}
                </div>
              ); })()}
            </div>
            ...
            {/* Client Name */}
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:11, color:T.muted, marginBottom:6, textTransform:"uppercase" }}>Client Name</div>
              <input value={convertForm.customerName} onChange={e=>setConvertForm({...convertForm,customerName:e.target.value})} style={{ width:"100%" }} />
              <div style={{ display:"flex", gap:8, marginTop:6, flexWrap:"wrap" }}>
                {clients.map(c => (
                  <button key={c.id} onClick={()=>setConvertForm({...convertForm,customerName:c.name})}
                    style={{ background:convertForm.customerName===c.name?T.accent+"22":"transparent", border:`1px solid ${convertForm.customerName===c.name?T.accent:T.border}`, borderRadius:6, padding:"3px 10px", color:convertForm.customerName===c.name?T.accent:T.muted, fontSize:11, cursor:"pointer" }}>
                    {c.name}
                  </button>
                ))}
              </div>
            ...
            <div style={{ background:T.surface, borderRadius:10, padding:14, marginBottom:16 }}>
              <div style={{ fontSize:12, color:T.yellow, fontWeight:700, marginBottom:10 }}>🚚 Freight Fee (FRT Supplier)</div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                <div>
                  <div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>FRT SUPPLIER</div>
                  <select value={convertForm.freightSupplierId} onChange={e=>setConvertForm({...convertForm,freightSupplierId:e.target.value})} style={{ width:"100%", fontSize:12 }}>
                    <option value="">None</option>
                    {suppliers.filter(s=>s.name.toUpperCase().includes("FRT")).map(s=>(
                      <option key={s.id} value={s.id}>{s.name}</option>
                    ))}
                ...
                <div>
                  <div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>FREIGHT %</div>
                  <input type="number" value={convertForm.freightPct} onChange={e=>setConvertForm({...convertForm,freightPct:e.target.value})} placeholder="0" min="0" max="100" />
                </div>
              </div>
              {convertForm.freightPct > 0 && (
                <div style={{ marginTop:10, padding:10, background:T.card, borderRadius:8, fontFamily:T.mono, fontSize:13 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                    <span style={{ color:T.muted }}>Subtotal</span>
                    <span>{fmt(convertModal.items.reduce((s,i)=>s+i.quantity*i.price,0))}</span>
                  </div>
                  <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                    <span style={{ color:T.yellow }}>Freight ({convertForm.freightPct}%)</span>
                    <span style={{ color:T.yellow }}>+{fmt(convertModal.items.reduce((s,i)=>s+i.quantity*i.price,0) * convertForm.freightPct / 100)}</span>
                  </div>
                  <div style={{ display:"flex", justifyContent:"space-between", fontWeight:800, borderTop:`1px solid ${T.border}`, paddingTop:6 }}>
                    <span>Invoice Total</span>
                    <span style={{ color:T.green }}>{fmt(convertModal.items.reduce((s,i)=>s+i.quantity*i.price,0) * (1 + convertForm.freightPct/100))}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={{ display:"flex", gap:10 }}>
              <Btn onClick={convertToInvoice} loading={converting===convertModal.id} color={T.green}>⚡ Create Invoice</Btn>
              <Btn outline onClick={()=>setConvertModal(null)}>Cancel</Btn>
            </div>
          </div>
        </div>
      )}

      {/* Orders List — grouped by Frete */}
      {loading ? <Loader /> : (() => {
        const freteSups = suppliers.filter(s => s.name && s.name.toUpperCase().includes("FRT"));
        const noFrete = orders.filter(o => !o.freight_supplier_id || !freteSups.find(s=>+s.id===+o.freight_supplier_id));
        const groups = [
          ...freteSups.map(s => ({ id: s.id, name: s.name, orders: orders.filter(o => +o.freight_supplier_id === +s.id) })).filter(g => g.orders.length > 0),
          ...(noFrete.length > 0 ? [{ id: "none", name: "No Frete Assigned", orders: noFrete }] : []),
        ];
        if (orders.length === 0) return <div style={{ textAlign:"center", padding:40, color:T.muted }}>No orders yet</div>;
        return (
          <div style={{ display:"flex", flexDirection:"column", gap:20 }}>
            {groups.map(group => {
              const groupTotal = group.orders.reduce((s,o) => s+(+o.total||0), 0);
              const groupQty = group.orders.reduce((s,o) => s+o.items.reduce((si,i)=>si+(+i.quantity),0), 0);
              const pending = group.orders.filter(o => o.status==="draft"||o.status==="confirmed").length;
              return (
                <div key={group.id}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:T.surface, border:`1px solid ${T.accent}44`, borderRadius:10, padding:"12px 18px", marginBottom:10 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                      <span style={{ fontSize:18 }}>🚚</span>
                      <div>
                        <div style={{ fontWeight:800, fontSize:15, color:T.accent }}>{group.name}</div>
                        <div style={{ fontSize:12, color:T.muted }}>{group.orders.length} order{group.orders.length!==1?"s":""} · {groupQty} pcs{pending>0?` · ${pending} pending`:""}</div>
                      </div>
                    </div>
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontFamily:T.mono, fontWeight:800, fontSize:16, color:T.green }}>{fmt(groupTotal)}</div>
                      <div style={{ fontSize:11, color:T.muted }}>Total value</div>
                    </div>
                  </div>
                  <div style={{ display:"flex", flexDirection:"column", gap:10, paddingLeft:16, borderLeft:`2px solid ${T.accent}33` }}>
            ...
            <div key={order.id} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, overflow: "hidden" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 20px", borderBottom: `1px solid ${T.border}`, flexWrap: "wrap", gap: 10 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 12, color: T.muted, fontFamily: T.mono }}>{order.items.reduce((s,i)=>s+(+i.quantity),0)} pcs</span>
                  ...
                  {(order.status === "draft" || order.status === "confirmed") && (
                    <Btn small onClick={() => openConvertModal(order)} color={T.green}>⚡ Invoice</Btn>
                  )}
                  <button onClick={() => printOrder(order)} style={{ background:T.green+"22", border:`1px solid ${T.green}44`, borderRadius:8, padding:"6px 12px", color:T.green, fontSize:12, cursor:"pointer" }}>🖨️</button>
                  ...
                  {order.status === "cancelled" && (
                    <button onClick={() => deleteOrder(order)} style={{ background: T.red+"33", border:`1px solid ${T.red}66`, borderRadius:8, padding:"6px 12px", color:T.red, fontSize:12, fontWeight:700, cursor:"pointer" }}>🗑️ Delete</button>
                  )}
                </div>
              </div>
              {order.items.length > 0 && (() => {
                const totalQty = order.items.reduce((s,i)=>s+(+i.quantity),0);
                const subtotal = order.items.reduce((s,i)=>s+i.quantity*i.price,0);
                const freightAmt = +(order.freight_pct||0) > 0 ? subtotal * (+(order.freight_pct||0)/100) : 0;
                const freightSupplier = order.freight_supplier_id ? suppliers.find(s=>+s.id===+order.freight_supplier_id) : null;
                return (
                  <div style={{ padding:"0" }}>
                    <table>
                      ...
                      <tbody>
                        ...
                        <tr style={{ background:T.surface, borderTop:`2px solid ${T.border}` }}>
                          <td style={{ fontWeight:700, color:T.muted, fontSize:12 }}>TOTAL</td>
                          <td style={{ fontFamily:T.mono, fontWeight:800, color:T.accent }}>{totalQty} pcs</td>
                          <td></td>
                          <td style={{ fontFamily:T.mono, fontWeight:800, color:T.accent }}>{fmt(subtotal)}</td>
                        </tr>
                        {/* Freight row */}
                        {freightAmt > 0 && (
                          <tr style={{ background:T.yellow+"11" }}>
                            <td colSpan={2} style={{ fontSize:12, color:T.yellow }}>🚚 Freight {order.freight_pct}% {freightSupplier?`— ${freightSupplier.name}`:""}</td>
                            <td></td>
                            <td style={{ fontFamily:T.mono, fontWeight:700, color:T.yellow }}>+{fmt(freightAmt)}</td>
                          </tr>
                        )}
                        {/* Grand Total with freight */}
                        {freightAmt > 0 && (
                          <tr style={{ background:T.green+"11" }}>
                            <td colSpan={2} style={{ fontWeight:800, fontSize:13 }}>GRAND TOTAL (incl. freight)</td>
                            <td></td>
                            <td style={{ fontFamily:T.mono, fontWeight:800, fontSize:15, color:T.green }}>{fmt(subtotal+freightAmt)}</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
...
function ExpensesPage({ locations, onRefresh }) {
  ...
  const [form, setForm] = useState({
    ...
    amount: "", location_id: "", profit_source: "net_profit", payment_method: "cash_usd"
  });
  ...
  const save = async () => {
    ...
    await supabase.from("expenses").insert({
      ...
      amount: +form.amount, location_id: form.location_id ? +form.location_id : null,
      profit_source: form.profit_source, payment_method: form.payment_method,
    });
    setForm({ date: new Date().toISOString().slice(0,10), category: "", description: "", amount: "", location_id: "", profit_source: "net_profit", payment_method: "cash_usd" });
    setShowAdd(false);
    await load();
    setSaving(false);
  };
  ...
  const filtered = filterType === "all" ? expenses : expenses.filter(e => e.profit_source === filterType);
  const totalNet = expenses.filter(e => e.profit_source === "net_profit" || !e.profit_source).reduce((s,e)=>s+(+e.amount),0);
  const totalBsm = expenses.filter(e => e.profit_source === "bsm_profit").reduce((s,e)=>s+(+e.amount),0);
  const totalCash = expenses.filter(e => e.payment_method === "cash_usd").reduce((s,e)=>s+(+e.amount),0);
  const totalUsdt = expenses.filter(e => e.payment_method === "wallet_usdt").reduce((s,e)=>s+(+e.amount),0);
  const total = expenses.reduce((s,e)=>s+(+e.amount),0);

  const profitSourceLabel = { net_profit: "📈 Net Profit", bsm_profit: "🏦 BSM Profit" };
  const profitSourceColor = { net_profit: T.green, bsm_profit: T.accent };
  const paymentLabel = { cash_usd: "💵 Cash", wallet_usdt: "💎 USDT" };
  const paymentColor = { cash_usd: T.green, wallet_usdt: T.accent };
  ...
  return (
    <div className="page">
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 14, marginBottom: 24 }}>
        <div style={{ background: T.card, border: `1px solid ${T.red}44`, borderRadius: 12, padding: 18 }}>
          <div style={{ fontSize: 11, color: T.muted, textTransform: "uppercase", marginBottom: 6 }}>Total Expenses</div>
          <div style={{ fontSize: 22, fontWeight: 800, color: T.red, fontFamily: T.mono }}>{fmt(total)}</div>
        </div>
        <div style={{ background: T.card, border: `1px solid ${T.green}44`, borderRadius: 12, padding: 18 }}>
          <div style={{ fontSize: 11, color: T.muted, textTransform: "uppercase", marginBottom: 6 }}>📈 From Net Profit</div>
          <div style={{ fontSize: 22, fontWeight: 800, color: T.green, fontFamily: T.mono }}>{fmt(totalNet)}</div>
        </div>
        <div style={{ background: T.card, border: `1px solid ${T.accent}44`, borderRadius: 12, padding: 18 }}>
          <div style={{ fontSize: 11, color: T.muted, textTransform: "uppercase", marginBottom: 6 }}>🏦 From BSM Profit</div>
          <div style={{ fontSize: 22, fontWeight: 800, color: T.accent, fontFamily: T.mono }}>{fmt(totalBsm)}</div>
        </div>
        <div style={{ background: T.card, border: `1px solid ${T.green}44`, borderRadius: 12, padding: 18 }}>
          <div style={{ fontSize: 11, color: T.muted, textTransform: "uppercase", marginBottom: 6 }}>💵 Cash USD</div>
          <div style={{ fontSize: 22, fontWeight: 800, color: T.green, fontFamily: T.mono }}>{fmt(totalCash)}</div>
        </div>
        <div style={{ background: T.card, border: `1px solid ${T.accent}44`, borderRadius: 12, padding: 18 }}>
          <div style={{ fontSize: 11, color: T.muted, textTransform: "uppercase", marginBottom: 6 }}>💎 Wallet USDT</div>
          <div style={{ fontSize: 22, fontWeight: 800, color: T.accent, fontFamily: T.mono }}>{fmt(totalUsdt)}</div>
        </div>
      </div>
      ...
      {showAdd && (
        <div style={{ background: T.card, border: `1px solid ${T.accent}44`, borderRadius: 12, padding: 24, marginBottom: 24 }}>
          ...
          {/* Profit Source */}
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 11, color: T.muted, marginBottom: 8, textTransform: "uppercase" }}>Deduct From</div>
            <div style={{ display: "flex", gap: 10 }}>
              {[["net_profit","📈 Net Profit",T.green],["bsm_profit","🏦 BSM Profit",T.accent]].map(([val,lbl,col]) => (
                <button key={val} onClick={() => setForm({...form, profit_source: val})}
                  style={{ padding:"8px 18px", borderRadius:10, border:`2px solid ${form.profit_source===val?col:T.border}`, background:form.profit_source===val?col+"22":"transparent", color:form.profit_source===val?col:T.muted, fontWeight:700, fontSize:13, cursor:"pointer" }}>
                  {lbl}
                </button>
              ))}
            </div>
          ...
          <div style={{ marginBottom: 16 }}>
            <div style={{ fontSize: 11, color: T.muted, marginBottom: 8, textTransform: "uppercase" }}>Payment Method</div>
            <div style={{ display: "flex", gap: 10 }}>
              {[["cash_usd","💵 Cash USD",T.green],["wallet_usdt","💎 Wallet USDT",T.accent]].map(([val,lbl,col]) => (
                <button key={val} onClick={() => setForm({...form, payment_method: val})}
                  style={{ padding:"8px 18px", borderRadius:10, border:`2px solid ${form.payment_method===val?col:T.border}`, background:form.payment_method===val?col+"22":"transparent", color:form.payment_method===val?col:T.muted, fontWeight:700, fontSize:13, cursor:"pointer" }}>
                  {lbl}
                </button>
              ))}
            </div>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            <Btn onClick={save} loading={saving}>Save Expense</Btn>
            <Btn outline onClick={() => setShowAdd(false)}>Cancel</Btn>
          </div>
        </div>
      )}
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16, flexWrap: "wrap" }}>
        {[["all","All"],["net_profit","📈 Net Profit"],["bsm_profit","🏦 BSM Profit"]].map(([val,lbl]) => (
          <button key={val} onClick={() => setFilterType(val)}
            style={{ padding:"5px 14px", borderRadius:8, border:`1px solid ${filterType===val?T.accent:T.border}`, background:filterType===val?T.accentDim:"transparent", color:filterType===val?T.accent:T.muted, fontSize:12, fontWeight:filterType===val?700:400, cursor:"pointer" }}>
            {lbl}
          </button>
        ))}
      </div>
      ...
      {loading ? <Loader /> : (
        <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, overflow: "hidden" }}>
          <table>
            <thead><tr><th>Date</th><th>Category</th><th>Description</th><th>Location</th><th>Deducted From</th><th>Payment</th><th>Amount</th><th></th></tr></thead>
            <tbody>
              {filtered.map(e => (
                <tr key={e.id}>
                  ...
                  <td><Badge color={profitSourceColor[e.profit_source]||T.green}>{profitSourceLabel[e.profit_source]||"📈 Net Profit"}</Badge></td>
                  <td><Badge color={paymentColor[e.payment_method]||T.green}>{paymentLabel[e.payment_method]||"💵 Cash"}</Badge></td>
              ...
              {filtered.length === 0 && <tr><td colSpan={8} style={{ textAlign: "center", color: T.muted, padding: 32 }}>No expenses found</td></tr>}
            </tbody>
          </table>
        </div>
      )}
...
// ─── REPORTS ──────────────────────────────────────────────────────────────────
function Reports({ invoices, products, locations, setPage, expenses = [] }) {
  const [period, setPeriod] = useState("all");
  const now = new Date();
  ...
  const netExpenses = filteredExpenses.filter(e=>e.profit_source==="net_profit"||!e.profit_source).reduce((s,e)=>s+(+e.amount),0);
  const bsmExpenses = filteredExpenses.filter(e=>e.profit_source==="bsm_profit").reduce((s,e)=>s+(+e.amount),0);
  const cashExpenses = filteredExpenses.filter(e=>e.payment_method==="cash_usd").reduce((s,e)=>s+(+e.amount),0);
  const usdtExpenses = filteredExpenses.filter(e=>e.payment_method==="wallet_usdt").reduce((s,e)=>s+(+e.amount),0);
  const netProfitAfterExpenses = profit - netExpenses;
  const bsmProfitAfterExpenses = bsmTotalProfit - bsmExpenses;
      ...
      {filteredExpenses.length > 0 && (
        <div style={{ marginTop: 24 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 14, marginBottom: 16 }}>
            <div style={{ background: T.card, border: `1px solid ${T.red}44`, borderRadius: 12, padding: 18 }}>
              <div style={{ fontSize: 11, color: T.muted, textTransform: "uppercase", marginBottom: 6 }}>Total Expenses</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: T.red, fontFamily: T.mono }}>{fmt(totalExpenses)}</div>
            </div>
            <div style={{ background: T.card, border: `1px solid ${T.green}44`, borderRadius: 12, padding: 18 }}>
              <div style={{ fontSize: 11, color: T.muted, textTransform: "uppercase", marginBottom: 6 }}>📈 Net Profit After Expenses</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: netProfitAfterExpenses>=0?T.green:T.red, fontFamily: T.mono }}>{fmt(netProfitAfterExpenses)}</div>
            </div>
            <div style={{ background: T.card, border: `1px solid ${T.accent}44`, borderRadius: 12, padding: 18 }}>
              <div style={{ fontSize: 11, color: T.muted, textTransform: "uppercase", marginBottom: 6 }}>🏦 BSM Profit After Expenses</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: bsmProfitAfterExpenses>=0?T.accent:T.red, fontFamily: T.mono }}>{fmt(bsmProfitAfterExpenses)}</div>
            </div>
            <div style={{ background: T.card, border: `1px solid ${T.green}44`, borderRadius: 12, padding: 18 }}>
              <div style={{ fontSize: 11, color: T.muted, textTransform: "uppercase", marginBottom: 6 }}>💵 Cash Expenses</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: T.green, fontFamily: T.mono }}>{fmt(cashExpenses)}</div>
            </div>
            <div style={{ background: T.card, border: `1px solid ${T.accent}44`, borderRadius: 12, padding: 18 }}>
              <div style={{ fontSize: 11, color: T.muted, textTransform: "uppercase", marginBottom: 6 }}>💎 USDT Expenses</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: T.accent, fontFamily: T.mono }}>{fmt(usdtExpenses)}</div>
            </div>
          </div>
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, overflow: "hidden" }}>
            <div style={{ padding: "14px 20px", borderBottom: `1px solid ${T.border}` }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: T.red }}>💸 Expenses Breakdown</h3>
            </div>
            <table>
              <thead><tr><th>Date</th><th>Category</th><th>Description</th><th>Deducted From</th><th>Payment</th><th>Amount</th></tr></thead>
              <tbody>
                {filteredExpenses.map((e, i) => (
                  <tr key={i}>
                    ...
                    <td><Badge color={T.yellow}>{e.category||"Other"}</Badge></td>
                    <td style={{ fontWeight: 600 }}>{e.description}</td>
                    <td><Badge color={e.profit_source==="bsm_profit"?T.accent:T.green}>{e.profit_source==="bsm_profit"?"🏦 BSM Profit":"📈 Net Profit"}</Badge></td>
                    <td><Badge color={e.payment_method==="wallet_usdt"?T.accent:T.green}>{e.payment_method==="wallet_usdt"?"💎 USDT":"💵 Cash"}</Badge></td>
                    <td style={{ fontFamily: T.mono, color: T.red, fontWeight: 700 }}>{fmt(e.amount)}</td>
                  </tr>
                ))}
              </tbody>
...
function ClientBalance({ clients, invoices, onRefresh }) {
  ...
  const refreshInvoices = async () => {
    ...
    // Fetch fresh invoices from DB, sorted oldest→newest
    ...
    const allInvoices = freshInvoices || [];
  ...
  const printClientPayment = (p, clientName) => {
    const html = `<!DOCTYPE html><html><head><title>Najib Cell — Payment Record</title><style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      <div class="footer">Thank you! • Najib Cell Management</div>
      <script>window.onload = function(){ window.print(); window.onafterprint = function(){ window.close(); }; }</script>
    </body></html>`;
  ...
  const printClientLedger = () => {
    ...
    const html = `<!DOCTYPE html><html><head><title>Ledger - ${clientName}</title><style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      <h1>📱 Najib Cell — Client Ledger</h1>
                  ...
                  return clientInvoices.map(inv => {
                    ...
                    return (
                      <tr key={inv.id}>
                        ...
                        <td style={{ fontFamily: T.mono, fontWeight: 700 }}>{fmt(inv.supplier_amount||inv.total)}{inv.shipment_value>0&&<span style={{fontSize:10,color:T.muted,display:"block"}}>+{fmt(inv.shipment_value)} ship</span>}</td>
...
function SupplierBalance({ suppliers, invoices, onRefresh }) {
  // Exclude shipping companies — they appear in Shipping Balance instead
  const productSuppliers = suppliers.filter(s => !s.is_shipping);
  const [selected, setSelected] = useState(null);
  const [payments, setPayments] = useState([]);
  const [showAdd, setShowAdd] = useState(false);
  const [saving, setSaving] = useState(false);
  ...
  // Use supplier_amount (excludes shipment) if available, otherwise use total
  const totalOwed = supplierInvoices.reduce((s,i)=>s+(i.supplier_amount||i.total),0);
  const totalPaid = payments.filter(p=>p.type==="payment").reduce((s,p)=>s+(+p.amount),0);
  const totalCharges = payments.filter(p=>p.type==="charge").reduce((s,p)=>s+(+p.amount),0);
  const balance = totalOwed + totalCharges - totalPaid;

  const refreshInvoices = async () => {
    // Bug fix 1: Always fetch fresh payments from DB (not stale prop)
    const { data: allPayments } = await supabase.from("supplier_payments").select("*").eq("supplier_id", selected);
    const newTotalPaid = (allPayments || []).filter(p => p.type === "payment").reduce((s, p) => s + (+p.amount), 0);

    // Fetch invoices fresh from DB, sorted oldest→newest
    // Must fetch supplier_amount — balance excludes shipment, compare against supplier_amount not total
    const { data: freshInvoices } = await supabase
      .from("invoices")
      .select("id, total, supplier_amount, date")
      .eq("type", "buy")
      .eq("supplier_id", selected)
      .order("date", { ascending: true });
    const allInvoices = freshInvoices || [];

    // Reset all invoices to pending first
    for (const inv of allInvoices) {
      await supabase.from("invoices").update({ payment_status: "pending", status: "pending", amount_paid: 0 }).eq("id", inv.id);
    }

    // Distribute payments oldest-first using supplier_amount (excludes shipment)
    let remaining = newTotalPaid;
    for (const inv of allInvoices) {
      if (remaining <= 0) break;
      const owed = inv.supplier_amount || inv.total;
      if (remaining >= owed) {
        await supabase.from("invoices").update({ payment_status: "paid", status: "paid", amount_paid: owed }).eq("id", inv.id);
        remaining -= owed;
      } else {
        await supabase.from("invoices").update({ payment_status: "partial", status: "partial", amount_paid: remaining }).eq("id", inv.id);
        remaining = 0;
  ...
  const save = async () => {
    if (!form.amount || !selected) return;
    setSaving(true);
    await supabase.from("supplier_payments").insert({ supplier_id: selected, date: form.date, amount: +form.amount, type: form.type, notes: form.notes, payment_method: form.type === "payment" ? (form.payment_method || "cash_usd") : null });
  ...
  const printSupplierPayment = (p, supplierName) => {
    const html = `<!DOCTYPE html><html><head><title>Najib Cell — Payment Voucher</title><style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      body { font-family: Arial, sans-serif; color: #000; background: white; padding: 40px; max-width: 400px; margin: 0 auto; }
      h1 { font-size: 20px; font-weight: 800; margin-bottom: 4px; }
      .subtitle { font-size: 13px; color: #666; margin-bottom: 20px; }
      ...
      <div class="footer">Najib Cell Management</div>
      <script>window.onload = function(){ window.print(); window.onafterprint = function(){ window.close(); }; }</script>
    </body></html>`;
    const w = window.open("", "_blank", "width=500,height=650");
    w.document.write(html);
  ...
  const printSupplierLedger = () => {
    const supplierName = productSuppliers.find(s => s.id === selected)?.name || "";
    const ledgerRows = [
      ...supplierInvoices.map(inv => ({ date: inv.date, desc: `Invoice ${inv.id}`, notes: inv.shipment_value>0?`Excl. shipment`:"", debit: inv.supplier_amount||inv.total, credit: 0 })),
      ...payments.filter(p => {
        if (p.type === "charge" && p.notes && p.notes.toLowerCase().includes("invoice")) return false;
        if (fromDate && p.date < fromDate) return false;
        if (toDate && p.date > toDate) return false;
        return true;
    ...
    const html = `<!DOCTYPE html><html><head><title>Ledger - ${supplierName}</title><style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      <h1>📱 Najib Cell — Supplier Ledger</h1>
  ...
  return (
    <div className="page">
          ...
          <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 12, overflow: "hidden", marginBottom: 20 }}>
            <table>
              ...
              <tbody>
                {(() => {
                  const ledgerRows = [
                    ...supplierInvoices.map(inv => ({ date: inv.date, desc: `Invoice ${inv.id}`, notes: inv.shipment_value>0?`Excl. shipment ${fmt(inv.shipment_value||0)}`:"", debit: inv.supplier_amount||inv.total, credit: 0, id: inv.id, isInv: true })),
                    ...payments.filter(p => !(p.type === "charge" && p.notes && p.notes.toLowerCase().includes("invoice"))).map(p => ({ date: p.date, desc: p.type === "payment" ? "Payment Made" : "Additional Charge", notes: p.notes || "", debit: p.type === "charge" ? +p.amount : 0, credit: p.type === "payment" ? +p.amount : 0, id: p.id, isInv: false, raw: p })),
                  ].sort((a, b) => a.date.localeCompare(b.date));
                  let runningBalance = 0;
                  return ledgerRows.map((row, i) => {
                    runningBalance += row.debit - row.credit;
            ...
            <table>
              ...
              <tbody>
                {(() => {
                  ...
                  const paidMap = {};
                  for (const inv of allInvsSorted) {
                    const invAmt = inv.supplier_amount||inv.total;
                    if (rem >= invAmt) { paidMap[inv.id] = invAmt; rem -= invAmt; }
                    else if (rem > 0) { paidMap[inv.id] = rem; rem = 0; }
                    else { paidMap[inv.id] = 0; }
                  }
      ...
      {!selected && productSuppliers.length > 0 && <div style={{ textAlign: "center", padding: 40, color: T.muted }}>Select a supplier above to view their balance sheet</div>}
      {productSuppliers.length === 0 && <div style={{ textAlign: "center", padding: 40, color: T.muted }}>No suppliers yet. Add suppliers first!</div>}
    </div>
  );
}
...
function ReceiptsPage({ clients }) {
  ...
  const printReceipt = (r) => {
    const html = `<!DOCTYPE html><html><head><title>Najib Cell — Receipt ${r.id}</title><style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      body { font-family: Arial, sans-serif; color: #000; background: white; padding: 40px; max-width: 400px; margin: 0 auto; }
      h1 { font-size: 20px; font-weight: 800; margin-bottom: 4px; }
      .subtitle { font-size: 13px; color: #666; margin-bottom: 20px; }
      ...
      <div class="footer">Thank you for your payment! • Najib Cell Management</div>
      <script>window.onload = function(){ window.print(); window.onafterprint = function(){ window.close(); }; }</script>
    </body></html>`;
...
function AccountLedger() {
  const [account, setAccount] = useState("cash_usd");
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  ...
  const ACCOUNTS = [
    { id: "cash_usd", label: "💵 Cash USD", color: "#22c55e" },
    { id: "wallet_usdt", label: "💎 Wallet USDT", color: "#00e5ff" },
    { id: "bank_transfer", label: "🏦 Bank Transfer", color: "#f59e0b" },
  ];

  const [openingBalances, setOpeningBalances] = useState(() => {
    try {
      // Check new key first, fall back to old key for migration
      const newKey = localStorage.getItem("BSM_opening_balances");
      const oldKey = localStorage.getItem("electropro_opening_balances");
      const data = JSON.parse(newKey || oldKey || "{}");
      // Migrate to new key if found in old key
      if (!newKey && oldKey) localStorage.setItem("BSM_opening_balances", oldKey);
      return data;
    } catch { return {}; }
  });
  ...
  const saveOpeningBalance = () => {
    const updated = { ...openingBalances, [account]: +tempOpening || 0 };
    setOpeningBalances(updated);
    localStorage.setItem("BSM_opening_balances", JSON.stringify(updated));
  ...
  const load = async () => {
    setLoading(true);
    // Load ALL transactions for this payment method from ALL tables
    const [r1, r1b, r2, r3, r4, r5, r6] = await Promise.all([
      supabase.from("receipts").select("id, date, amount, notes, payment_method, clients(name)").eq("payment_method", account),
      supabase.from("client_payments").select("id, date, amount, notes, payment_method, client_id").eq("payment_method", account).eq("type", "payment"),
      supabase.from("supplier_payments").select("id, date, amount, notes, payment_method, type, suppliers(name)").eq("payment_method", account).eq("type", "payment"),
      supabase.from("expenses").select("id, date, amount, description, category, payment_method").eq("payment_method", account),
      supabase.from("bsm_payments").select("id, date, amount, notes, payment_method, person").eq("payment_method", account),
      supabase.from("najib_orabi_payments").select("id, date, amount, notes, payment_method, person").eq("payment_method", account),
      supabase.from("capital_contributions").select("id, date, amount, method, notes, owner").eq("method", account),
    ]);
    const receiptKeys = new Set((r1.data||[]).map(x => `${x.date}_${+x.amount}_${x.client_id}`));
    const allRows = [
      ...(r1.data||[]).map(x => ({ date: x.date, desc: `Receipt — ${x.clients?.name||"Client"}`, notes: x.notes||"", credit: +x.amount, debit: 0, src: "receipt", uid: `receipt_${x.id}` })),
      ...(r1b.data||[]).filter(x => !receiptKeys.has(`${x.date}_${+x.amount}_${x.client_id}`)).map(x => ({ date: x.date, desc: `Client Payment`, notes: x.notes||"", credit: +x.amount, debit: 0, src: "client_pay", uid: `cpay_${x.id}` })),
      ...(r6.data||[]).map(x => ({ date: x.date, desc: `Capital Contribution${x.owner?" — "+x.owner:""}`, notes: x.notes||"", credit: +x.amount, debit: 0, src: "capital", uid: `cap_${x.id}` })),
      ...(r2.data||[]).map(x => ({ date: x.date, desc: `Payment — ${x.suppliers?.name||"Supplier"}`, notes: x.notes||"", credit: 0, debit: +x.amount, src: "sup_pay", uid: `spay_${x.id}` })),
      ...(r3.data||[]).map(x => ({ date: x.date, desc: `Expense — ${x.description}`, notes: x.category||"", credit: 0, debit: +x.amount, src: "expense", uid: `exp_${x.id}` })),
      ...(r4.data||[]).map(x => ({ date: x.date, desc: `BSM Profit — ${x.person==="basem"?"Basem":"Chadi"}`, notes: x.notes||"", credit: 0, debit: +x.amount, src: "bsm", uid: `bsm_${x.id}` })),
      ...(r5.data||[]).map(x => ({ date: x.date, desc: `Najib-Orabi Profit — ${x.person==="najib"?"Najib":"Orabi"}`, notes: x.notes||"", credit: 0, debit: +x.amount, src: "no_profit", uid: `no_${x.id}` })),
    ];
    const seen = new Set();
    const finalRows = allRows
      .filter(r => { if (seen.has(r.uid)) return false; seen.add(r.uid); return true; })
      .sort((a, b) => a.date.localeCompare(b.date));
  ...
  const typeColor = (r) => r.credit > 0 ? T.green : r.src === "expense" ? T.yellow : T.red;
  const typeLabel = (r) => r.credit > 0 ? "IN" : r.src === "expense" ? "EXP" : "OUT";
  ...
  const printLedger = () => {
    ...
    const rowsHtml = ledger.map(r => {
      return `<tr>
        ...
        <td>${r.desc}</td>
        <td>${r.notes||"—"}</td>
        <td style="color:#cc0000;text-align:right">${r.debit>0?"$"+r.debit.toFixed(2):"—"}</td>
        <td style="color:#006600;text-align:right">${r.credit>0?"$"+r.credit.toFixed(2):"—"}</td>
        <td style="font-weight:800;text-align:right;color:${r.balance>=0?"#006600":"#cc0000"}">${"$"+Math.abs(r.balance).toFixed(2)+" "+(r.balance>=0?"CR":"DR")}</td>
      </tr>`;
    }).join("");
    const period = fromDate||toDate ? ` (${fromDate||"start"} → ${toDate||"today"})` : " (All Time)";
    const html = `<!DOCTYPE html><html><head><title>${acct?.label} Ledger</title>
    <style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;padding:30px;font-size:12px;}
    h1{font-size:18px;font-weight:800;margin-bottom:4px;}h2{font-size:14px;color:#666;margin-bottom:20px;}
    table{width:100%;border-collapse:collapse;}th{background:#000;color:#fff;padding:8px;text-align:left;}
    td{padding:7px 8px;border-bottom:1px solid #eee;}tr:nth-child(even){background:#f9f9f9;}
    .footer{margin-top:20px;text-align:center;font-size:10px;color:#999;}</style></head>
    <body><h1>📱 Najib Cell — Account Ledger</h1><h2>${acct?.label}${period}</h2>
    <table><thead><tr><th>Date</th><th>Description</th><th>Notes</th><th>Debit (OUT)</th><th>Credit (IN)</th><th>Balance</th></tr></thead>
    <tbody>${rowsHtml}</tbody></table>
    <div class="footer">Total IN: $${totalCredit.toFixed(2)} | Total OUT: $${totalDebit.toFixed(2)} | Balance: $${Math.abs(balance).toFixed(2)} ${balance>=0?"CR":"DR"} • Printed: ${new Date().toLocaleDateString()}</div>
    <script>window.onload=function(){window.print();window.onafterprint=function(){window.close();}}</script>
    </body></html>`;
    const w = window.open("","_blank","width=900,height=700");
    w.document.write(html);
  ...
  return (
    <div className="page">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        <div>
          <h2 style={{ fontSize:28, fontWeight:800 }}>Account Ledger</h2>
          <p style={{ color:T.muted, fontSize:13, marginTop:4 }}>All transactions by payment method</p>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:12, marginBottom:24, flexWrap:"wrap" }}>
        {ACCOUNTS.map(a => (
          <button key={a.id} onClick={() => setAccount(a.id)}
            style={{ padding:"12px 28px", borderRadius:12, border:`2px solid ${account===a.id?a.color:T.border}`, background:account===a.id?a.color+"22":T.card, color:account===a.id?a.color:T.muted, fontWeight:700, fontSize:15, cursor:"pointer", transition:"all .15s" }}>
            {a.label}
          </button>
        ))}
      </div>
      ...
      {showOpeningBalance && (
        <div style={{ background:T.card, border:`1px solid ${acct?.color}44`, borderRadius:12, padding:20, marginBottom:16 }}>
          <div style={{ fontSize:13, fontWeight:700, color:acct?.color, marginBottom:12 }}>💰 Set Opening Balance for {acct?.label}</div>
          <div style={{ fontSize:12, color:T.muted, marginBottom:10 }}>Enter how much cash/USDT you had before the transactions recorded here.</div>
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            <input type="number" value={tempOpening} onChange={e=>setTempOpening(e.target.value)} placeholder="0.00" style={{ maxWidth:200 }} />
            <Btn small onClick={saveOpeningBalance}>Save</Btn>
            <Btn small outline onClick={()=>setShowOpeningBalance(false)}>Cancel</Btn>
          </div>
        </div>
      )}
      ...
      {/* Summary Cards */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:16, marginBottom:24 }}>
        <div style={{ background:T.card, border:`1px solid ${T.accent}44`, borderRadius:12, padding:20, cursor:"pointer" }} onClick={()=>{ setTempOpening(openingBalance||""); setShowOpeningBalance(true); }}>
          <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:8 }}>🏦 Opening Balance</div>
          <div style={{ fontSize:22, fontWeight:800, color:T.accent, fontFamily:T.mono }}>{fmt(openingBalance)}</div>
          <div style={{ fontSize:10, color:T.muted, marginTop:4 }}>click to edit</div>
        </div>
        <div style={{ background:T.card, border:`1px solid ${T.green}44`, borderRadius:12, padding:20 }}>
          <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:8 }}>💰 Total In (Credit)</div>
          <div style={{ fontSize:22, fontWeight:800, color:T.green, fontFamily:T.mono }}>{fmt(totalCredit)}</div>
        </div>
        <div style={{ background:T.card, border:`1px solid ${T.red}44`, borderRadius:12, padding:20 }}>
          <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:8 }}>💸 Total Out (Debit)</div>
          <div style={{ fontSize:22, fontWeight:800, color:T.red, fontFamily:T.mono }}>{fmt(totalDebit)}</div>
        </div>
        <div style={{ background:T.card, border:`1px solid ${balance>=0?T.green:T.red}44`, borderRadius:12, padding:20 }}>
          <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:8 }}>⚖️ Current Balance</div>
          <div style={{ fontSize:22, fontWeight:800, color:balance>=0?T.green:T.red, fontFamily:T.mono }}>{fmt(Math.abs(balance))} {balance>=0?"CR":"DR"}</div>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap", background:T.card, border:`1px solid ${T.border}`, borderRadius:10, padding:"10px 14px", marginBottom:16 }}>
        ...
        <input type="date" value={toDate} onChange={e=>setToDate(e.target.value)} style={{ width:"auto" }} />
        <button onClick={()=>{setFromDate("");setToDate("");}} style={{ background:"transparent", border:`1px solid ${T.border}`, borderRadius:6, padding:"4px 10px", color:T.muted, fontSize:11, cursor:"pointer" }}>Clear</button>
        <button onClick={load} style={{ background:T.accent+"22", border:`1px solid ${T.accent}44`, borderRadius:8, padding:"6px 14px", color:T.accent, fontSize:12, fontWeight:700, cursor:"pointer" }}>🔄 Refresh</button>
        <button onClick={printLedger} style={{ marginLeft:"auto", background:T.green+"22", border:`1px solid ${T.green}44`, borderRadius:8, padding:"6px 14px", color:T.green, fontSize:12, fontWeight:700, cursor:"pointer" }}>🖨️ Print Ledger</button>
      </div>
      ...
      {loading ? <Loader /> : (
        <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:12, overflow:"hidden" }}>
          <table>
            <thead>
              <tr>
                <th>Date</th><th>Description</th><th>Notes</th><th>Type</th>
                <th style={{ color:T.red }}>Debit (OUT)</th>
                <th style={{ color:T.green }}>Credit (IN)</th>
                <th style={{ color:acct?.color }}>Balance</th>
              </tr>
            </thead>
            <tbody>
              {openingBalance > 0 && (
                <tr style={{ background: T.accent+"11" }}>
                  <td style={{ fontFamily:T.mono, fontSize:12, color:T.muted }}>—</td>
                  <td style={{ fontWeight:700, color:T.accent }}>Opening Balance</td>
                  <td style={{ fontSize:12, color:T.muted }}>Starting balance</td>
                  <td><Badge color={T.accent}>OPEN</Badge></td>
                  <td style={{ fontFamily:T.mono, color:T.muted }}>—</td>
                  <td style={{ fontFamily:T.mono, color:T.accent, fontWeight:700 }}>{fmt(openingBalance)}</td>
                  <td style={{ fontFamily:T.mono, fontWeight:800, color:T.accent }}>{fmt(openingBalance)} CR</td>
                </tr>
              )}
              {ledger.map((row, i) => (
                <tr key={i}>
                  <td style={{ fontFamily:T.mono, fontSize:12 }}>{row.date}</td>
                  <td style={{ fontWeight:600, fontSize:13 }}>{row.desc}</td>
                  <td style={{ fontSize:12, color:T.muted }}>{row.notes||"—"}</td>
                  <td><Badge color={typeColor(row)}>{typeLabel(row)}</Badge></td>
                  <td style={{ fontFamily:T.mono, color:T.red, fontWeight:700 }}>{row.debit>0?fmt(row.debit):"—"}</td>
                  <td style={{ fontFamily:T.mono, color:T.green, fontWeight:700 }}>{row.credit>0?fmt(row.credit):"—"}</td>
                  <td style={{ fontFamily:T.mono, fontWeight:800, color:row.balance>=0?T.green:T.red }}>{fmt(Math.abs(row.balance))} {row.balance>=0?"CR":"DR"}</td>
                </tr>
              ))}
              {ledger.length===0 && <tr><td colSpan={7} style={{ textAlign:"center", color:T.muted, padding:40 }}>No {acct?.label} transactions found</td></tr>}
            </tbody>
          </table>
        </div>
      )}
...
function CapitalPage({ invoices, expenses, locations }) {
  const OWNERS = [
    { id: "orabi", name: "Orabi", pct: 50, color: "#00e5ff" },
    { id: "najib", name: "Najib", pct: 50, color: "#a855f7" },
  ];
  ...
  const [capForm, setCapForm] = useState({ owner: "orabi", amount: "", date: new Date().toISOString().slice(0,10), notes: "", method: "cash_usd" });
  const [distForm, setDistForm] = useState({ date: new Date().toISOString().slice(0,10), notes: "", method: "cash_usd", owner: "both", customAmount: "" });
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  const [saving, setSaving] = useState(false);
  ...
  const loadData = async () => {
    ...
    const [c, d] = await Promise.all([
      ...
      supabase.from("profit_distributions").select("*").order("date", { ascending: false }),
    ]);
  ...
  const saveCapital = async () => {
    if (!capForm.amount) return;
    setSaving(true);
    const { error } = await supabase.from("capital_contributions").insert({
      owner: capForm.owner, amount: +capForm.amount,
      date: capForm.date, notes: capForm.notes || null, method: capForm.method,
    });
    if (error) {
      alert("Error saving: " + error.message + "\n\nMake sure you ran the SQL to create the capital_contributions table in Supabase!");
      setSaving(false);
      return;
    }
    setCapForm({ owner: "orabi", amount: "", date: new Date().toISOString().slice(0,10), notes: "", method: "cash_usd" });
  ...
  const distributeProfit = async () => {
    setSaving(true);
    const amount = distForm.owner === "both" ? undistributed : (+distForm.customAmount || 0);
    if (amount <= 0) { setSaving(false); return; }
    const orabiAmt = distForm.owner === "orabi" ? amount : distForm.owner === "both" ? amount * 0.5 : 0;
    const najibAmt = distForm.owner === "najib" ? amount : distForm.owner === "both" ? amount * 0.5 : 0;
    await supabase.from("profit_distributions").insert({
      amount, orabi: orabiAmt, najib: najibAmt,
      date: distForm.date, notes: distForm.notes || null, method: distForm.method,
    });
    setShowDistribute(false);
    setDistForm({ date: new Date().toISOString().slice(0,10), notes: "", method: "cash_usd", owner: "both", customAmount: "" });
    await loadData();
    setSaving(false);
  };
  ...
  const najibOrabiLocIds = (locations||[]).filter(l => l.name && l.name.toLowerCase().includes("najib")).map(l => l.id);
  const sells = invoices.filter(i => i.type === "sell" && !najibOrabiLocIds.includes(i.location_id));
  const revenue = sells.reduce((s, i) => s + i.total, 0);
  const cogs = sells.reduce((s, i) => s + i.cogs, 0);
  const bsmExpenses = (expenses||[]).filter(e => !e.location_id || !najibOrabiLocIds.includes(+e.location_id));
  const totalExpenses = bsmExpenses.reduce((s, e) => s + +e.amount, 0);
  const netProfit = revenue - cogs - totalExpenses;

  // Capital totals
  const totalCapital = contributions.reduce((s, c) => s + +c.amount, 0);
  const capitalByOwner = {};
  OWNERS.forEach(o => { capitalByOwner[o.id] = contributions.filter(c => c.owner === o.id).reduce((s, c) => s + +c.amount, 0); });

  // Distribution totals
  const totalDistributed = distributions.reduce((s, d) => s + +d.amount, 0);
  const distributedByOwner = {};
  OWNERS.forEach(o => { distributedByOwner[o.id] = distributions.reduce((s, d) => s + (+d[o.id]||0), 0); });

  const undistributed = netProfit - totalDistributed;

  return (
    <div className="page">
      <h2 style={{ fontSize:28, fontWeight:800, marginBottom:6 }}>💼 Capital & Profit</h2>
      <p style={{ color:T.muted, fontSize:13, marginBottom:24 }}>Track capital and distribute profits — Orabi 50% / Najib 50%</p>

      {/* Date Filter */}
      <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap", background:T.card, border:`1px solid ${T.border}`, borderRadius:10, padding:"10px 14px", marginBottom:20 }}>
        <span style={{ fontSize:11, color:T.muted, textTransform:"uppercase" }}>📅 From:</span>
        <input type="date" value={fromDate} onChange={e=>setFromDate(e.target.value)} style={{ width:"auto" }} />
        <span style={{ fontSize:11, color:T.muted, textTransform:"uppercase" }}>To:</span>
        <input type="date" value={toDate} onChange={e=>setToDate(e.target.value)} style={{ width:"auto" }} />
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:14, marginBottom:24 }}>
        <div style={{ background:T.card, border:`1px solid ${T.accent}44`, borderRadius:12, padding:18 }}>
          <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:6 }}>💰 Total Capital</div>
          <div style={{ fontSize:22, fontWeight:800, color:T.accent, fontFamily:T.mono }}>{fmt(totalCapital)}</div>
        </div>
        <div style={{ background:T.card, border:`1px solid ${T.green}44`, borderRadius:12, padding:18 }}>
          <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:6 }}>📈 Net Profit</div>
          <div style={{ fontSize:22, fontWeight:800, color:netProfit>=0?T.green:T.red, fontFamily:T.mono }}>{fmt(netProfit)}</div>
        </div>
        <div style={{ background:T.card, border:`1px solid ${T.yellow}44`, borderRadius:12, padding:18 }}>
          <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:6 }}>✅ Distributed</div>
          <div style={{ fontSize:22, fontWeight:800, color:T.yellow, fontFamily:T.mono }}>{fmt(totalDistributed)}</div>
        </div>
        <div style={{ background:T.card, border:`1px solid ${undistributed>=0?T.green:T.red}44`, borderRadius:12, padding:18 }}>
          <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:6 }}>⏳ Undistributed</div>
          <div style={{ fontSize:22, fontWeight:800, color:undistributed>=0?T.green:T.red, fontFamily:T.mono }}>{fmt(undistributed)}</div>
        </div>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, marginBottom:24 }}>
        {OWNERS.map(o => (
          <div key={o.id} style={{ background:T.card, border:`2px solid ${o.color}44`, borderRadius:16, padding:24 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
              <div>
                <div style={{ fontSize:20, fontWeight:800, color:o.color }}>{o.name}</div>
                <div style={{ fontSize:13, color:T.muted }}>Partner • {o.pct}%</div>
              </div>
              <div style={{ width:50, height:50, borderRadius:"50%", background:o.color+"22", border:`2px solid ${o.color}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, fontWeight:800, color:o.color }}>{o.name[0]}</div>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
              {[
                { label:"Capital Invested", val: capitalByOwner[o.id]||0, color: o.color },
                { label:"Profit Share", val: netProfit*(o.pct/100), color: T.green },
                { label:"Received", val: distributedByOwner[o.id]||0, color: T.yellow },
                { label:"Pending", val: Math.max(0,(netProfit*(o.pct/100))-(distributedByOwner[o.id]||0)), color: T.red },
              ].map((item, i) => (
                <div key={i} style={{ background:T.surface, borderRadius:8, padding:12 }}>
                  <div style={{ fontSize:10, color:T.muted, marginBottom:4 }}>{item.label}</div>
                  <div style={{ fontSize:16, fontWeight:700, color:item.color, fontFamily:T.mono }}>{fmt(item.val)}</div>
                </div>
              ))}
            </div>
          </div>
      ...
      {/* Action Buttons */}
      <div style={{ display:"flex", gap:12, marginBottom:24, flexWrap:"wrap" }}>
        <Btn onClick={()=>setShowAddCapital(!showAddCapital)}>+ Add Capital</Btn>
        {undistributed > 0 && <Btn onClick={()=>setShowDistribute(!showDistribute)} color={T.green}>💸 Distribute {fmt(undistributed)}</Btn>}
      </div>

      {/* Add Capital Form */}
      {showAddCapital && (
        <div style={{ background:T.card, border:`1px solid ${T.accent}44`, borderRadius:12, padding:20, marginBottom:20 }}>
          <h3 style={{ fontSize:14, fontWeight:700, color:T.accent, marginBottom:14 }}>💰 Add Capital Contribution</h3>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:12 }}>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>OWNER</div>
              <select value={capForm.owner} onChange={e=>setCapForm({...capForm,owner:e.target.value})}>
                {OWNERS.map(o => <option key={o.id} value={o.id}>{o.name}</option>)}
              </select>
            </div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>DATE</div>
              <input type="date" value={capForm.date} onChange={e=>setCapForm({...capForm,date:e.target.value})} />
            </div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>AMOUNT ($)</div>
              <input type="number" value={capForm.amount} onChange={e=>setCapForm({...capForm,amount:e.target.value})} placeholder="0.00" />
            </div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>METHOD</div>
            ...
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>NOTES</div>
              <input value={capForm.notes} onChange={e=>setCapForm({...capForm,notes:e.target.value})} placeholder="Optional" />
            </div>
          </div>
          <div style={{ display:"flex", gap:10, marginTop:12 }}>
            <Btn small onClick={saveCapital} loading={saving}>Save</Btn>
            <Btn small outline onClick={()=>setShowAddCapital(false)}>Cancel</Btn>
          </div>
        </div>
      )}
      ...
      {showDistribute && (
        <div style={{ background:T.card, border:`1px solid ${T.green}44`, borderRadius:12, padding:20, marginBottom:20 }}>
          <h3 style={{ fontSize:14, fontWeight:700, color:T.green, marginBottom:14 }}>💸 Pay Profit to Owner</h3>

          {/* Owner selector */}
          <div style={{ marginBottom:14 }}>
            <div style={{ fontSize:11, color:T.muted, marginBottom:8, textTransform:"uppercase" }}>Pay To</div>
            <div style={{ display:"flex", gap:10, flexWrap:"wrap" }}>
              {[
                { val:"both", label:"👥 Both Owners (50/50)", color:T.green },
                ...OWNERS.map(o => ({ val:o.id, label:`${o.name} only`, color:o.color }))
              ].map(opt => (
                <button key={opt.val} onClick={()=>setDistForm({...distForm,owner:opt.val,customAmount:""})}
                  style={{ padding:"8px 16px", borderRadius:10, border:`2px solid ${distForm.owner===opt.val?opt.color:T.border}`, background:distForm.owner===opt.val?opt.color+"22":"transparent", color:distForm.owner===opt.val?opt.color:T.muted, fontWeight:700, fontSize:13, cursor:"pointer" }}>
                  {opt.label}
                </button>
              ))}
            </div>
          ...
          {/* Amount preview */}
          <div style={{ background:T.surface, borderRadius:10, padding:14, marginBottom:14 }}>
            {distForm.owner === "both" ? (
              <>
                {OWNERS.map(o => (
                  <div key={o.id} style={{ display:"flex", justifyContent:"space-between", padding:"8px 0", borderBottom:`1px solid ${T.border}` }}>
                    <span style={{ color:o.color, fontWeight:700 }}>{o.name} ({o.pct}%)</span>
                    <span style={{ fontFamily:T.mono, fontWeight:800, color:T.green }}>{fmt(undistributed * o.pct / 100)}</span>
                  </div>
                ))}
                <div style={{ display:"flex", justifyContent:"space-between", padding:"8px 0", fontWeight:800 }}>
                  <span>Total</span><span style={{ fontFamily:T.mono, color:T.green }}>{fmt(undistributed)}</span>
                </div>
              </>
            ) : (
              <div>
                <div style={{ fontSize:11, color:T.muted, marginBottom:8 }}>
                  {OWNERS.find(o=>o.id===distForm.owner)?.name}'s share: {fmt(undistributed * 0.5)} available
                </div>
                <div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>AMOUNT TO PAY ($)</div>
                <input type="number" value={distForm.customAmount}
                  onChange={e=>setDistForm({...distForm,customAmount:e.target.value})}
                  placeholder={`Max: ${(undistributed*0.5).toFixed(2)}`}
                  max={undistributed*0.5} min="0"
                  style={{ maxWidth:200 }} />
                {+distForm.customAmount > 0 && (
                  <div style={{ marginTop:8, fontSize:13, fontFamily:T.mono, color:T.green, fontWeight:700 }}>
                    Paying: {fmt(+distForm.customAmount)} to {OWNERS.find(o=>o.id===distForm.owner)?.name}
                  </div>
                )}
              </div>
            )}
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:12 }}>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>DATE</div>
              <input type="date" value={distForm.date} onChange={e=>setDistForm({...distForm,date:e.target.value})} />
            </div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>PAYMENT METHOD</div>
            ...
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>NOTES</div>
              <input value={distForm.notes} onChange={e=>setDistForm({...distForm,notes:e.target.value})} placeholder="Optional" />
            </div>
          </div>
          <div style={{ display:"flex", gap:10, marginTop:12 }}>
            <Btn small onClick={distributeProfit} loading={saving} color={T.green}>✅ Confirm Payment</Btn>
            <Btn small outline onClick={()=>setShowDistribute(false)}>Cancel</Btn>
          </div>
        </div>
      )}
      ...
      {/* Capital Table */}
      <div style={{ marginBottom:24 }}>
        <h3 style={{ fontSize:13, fontWeight:700, color:T.accent, textTransform:"uppercase", letterSpacing:1, marginBottom:10 }}>💰 Capital Contributions</h3>
        <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:12, overflow:"hidden" }}>
          {loading ? <Loader /> : (
            <table>
              <thead><tr><th>Date</th><th>Owner</th><th>Amount</th><th>Method</th><th>Notes</th><th></th></tr></thead>
              <tbody>
                {filterDate(contributions).map(c => {
                  const o = OWNERS.find(o=>o.id===c.owner);
                  return (
                    <tr key={c.id}>
                      <td style={{ fontFamily:T.mono, fontSize:12 }}>{c.date}</td>
                      <td style={{ fontWeight:700, color:o?.color }}>{o?.name}</td>
                      <td style={{ fontFamily:T.mono, fontWeight:700, color:T.accent }}>{fmt(c.amount)}</td>
                      <td><Badge color={c.method==="wallet_usdt"?T.accent:c.method==="bank_transfer"?T.yellow:T.green}>{c.method==="cash_usd"?"💵 Cash":c.method==="wallet_usdt"?"💎 USDT":"🏦 Bank"}</Badge></td>
                      <td style={{ color:T.muted, fontSize:12 }}>{c.notes||"—"}</td>
                      <td><button onClick={()=>deleteContribution(c.id)} style={{ background:T.red+"22", border:`1px solid ${T.red}44`, borderRadius:6, padding:"4px 8px", color:T.red, fontSize:11, cursor:"pointer" }}>🗑️</button></td>
                    </tr>
                  );
                })}
                {contributions.length===0 && <tr><td colSpan={6} style={{ textAlign:"center", color:T.muted, padding:24 }}>No capital contributions yet</td></tr>}
              </tbody>
            </table>
          )}
        </div>
      ...
      {/* Distributions Table */}
      <div>
        <h3 style={{ fontSize:13, fontWeight:700, color:T.green, textTransform:"uppercase", letterSpacing:1, marginBottom:10 }}>💸 Profit Distributions</h3>
        <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:12, overflow:"hidden" }}>
          {loading ? <Loader /> : (
            <table>
              <thead><tr><th>Date</th><th>Total</th><th>Orabi (50%)</th><th>Najib (50%)</th><th>Method</th><th>Notes</th><th></th></tr></thead>
              <tbody>
                {filterDate(distributions).map(d => (
                  <tr key={d.id}>
                    <td style={{ fontFamily:T.mono, fontSize:12 }}>{d.date}</td>
                    <td style={{ fontFamily:T.mono, fontWeight:800, color:T.green }}>{fmt(d.amount)}</td>
                    <td style={{ fontFamily:T.mono, color:OWNERS[0].color }}>{fmt(d.orabi||0)}</td>
                    <td style={{ fontFamily:T.mono, color:OWNERS[1].color }}>{fmt(d.najib||0)}</td>
                    <td><Badge color={d.method==="wallet_usdt"?T.accent:d.method==="bank_transfer"?T.yellow:T.green}>{d.method==="cash_usd"?"💵 Cash":d.method==="wallet_usdt"?"💎 USDT":"🏦 Bank"}</Badge></td>
                    <td style={{ color:T.muted, fontSize:12 }}>{d.notes||"—"}</td>
                    <td><button onClick={()=>deleteDist(d.id)} style={{ background:T.red+"22", border:`1px solid ${T.red}44`, borderRadius:6, padding:"4px 8px", color:T.red, fontSize:11, cursor:"pointer" }}>🗑️</button></td>
                  </tr>
                ))}
                {distributions.length===0 && <tr><td colSpan={7} style={{ textAlign:"center", color:T.muted, padding:24 }}>No distributions yet</td></tr>}
              </tbody>
            </table>
          )}
        </div>
...
// ─── Balance Sheet ────────────────────────────────────────────────────────────
function BalanceSheet({ invoices, expenses, suppliers, clients, locations }) {
  const [asOfDate, setAsOfDate] = useState(new Date().toISOString().slice(0,10));
  const [allData, setAllData]   = useState({ sp:[], receipts:[], capital:[], bsmPayments:[], najibOrabiPayments:[], expenses:[] });
  const [loading, setLoading]   = useState(true);

  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const [sp, r, cp, cap, bsmPay, noPay, exp] = await Promise.all([
        supabase.from("supplier_payments").select("*, suppliers(name, is_shipping)"),
        supabase.from("receipts").select("*"),
        supabase.from("client_payments").select("*").eq("type", "payment"),
        supabase.from("capital_contributions").select("*"),
        supabase.from("bsm_payments").select("*"),
        supabase.from("najib_orabi_payments").select("*"),
        supabase.from("expenses").select("*"),
      ]);
      setAllData({
        sp: sp.data||[],
        receipts: [...(r.data||[]), ...(cp.data||[]).map(p=>({...p, payment_method:p.payment_method||"wallet_usdt"}))],
        capital: cap.data||[], bsmPayments: bsmPay.data||[],
        najibOrabiPayments: noPay.data||[], expenses: exp.data||[],
      });
      setLoading(false);
    };
    load();
  ...
  const fd = items => items.filter(i => !asOfDate || (i.date||"") <= asOfDate);
  const filtInv = invoices.filter(i => !asOfDate || (i.date||"") <= asOfDate);
  const filtExp = fd(allData.expenses);
  const filtSP  = fd(allData.sp).filter(p => p.type === "payment");
  const filtR   = fd(allData.receipts);
  const filtCap = fd(allData.capital);
  const filtBsm = fd(allData.bsmPayments);
  const filtNo  = fd(allData.najibOrabiPayments);
  const sellInv = filtInv.filter(i => i.type === "sell");
  const buyInv  = filtInv.filter(i => i.type === "buy");

  const calcBal = (method) => (
    filtR.filter(r=>r.payment_method===method).reduce((s,r)=>s+(+r.amount||0),0) +
    filtCap.filter(c=>c.method===method).reduce((s,c)=>s+(+c.amount||0),0) -
    filtSP.filter(p=>p.payment_method===method).reduce((s,p)=>s+(+p.amount||0),0) -
    filtExp.filter(e=>e.payment_method===method).reduce((s,e)=>s+(+e.amount||0),0) -
    filtBsm.filter(p=>p.payment_method===method).reduce((s,p)=>s+(+p.amount||0),0) -
    filtNo.filter(p=>p.payment_method===method).reduce((s,p)=>s+(+p.amount||0),0)
  );
  const cashBal = calcBal("cash_usd");
  const usdtBal = calcBal("wallet_usdt");
  const bankBal = calcBal("bank_transfer");

  const clientMap = {};
  sellInv.forEach(inv => { const k=inv.customer||"Unknown"; if(!clientMap[k])clientMap[k]={invoiced:0,received:0}; clientMap[k].invoiced+=(+inv.total||0); });
  filtR.forEach(r => { const cli=clients.find(c=>c.id===r.client_id); const k=cli?.name||"Unknown"; if(!clientMap[k])clientMap[k]={invoiced:0,received:0}; clientMap[k].received+=(+r.amount||0); });
  const clientBalances = Object.entries(clientMap).map(([name,c])=>({name,balance:c.invoiced-c.received})).filter(c=>Math.abs(c.balance)>0.01).sort((a,b)=>b.balance-a.balance);
  const totalAR = clientBalances.reduce((s,c)=>s+c.balance,0);

  const inventoryVal = Math.max(0, buyInv.reduce((s,i)=>s+(+i.cogs||0),0) - sellInv.reduce((s,i)=>s+(+i.cogs||0),0));

  const supMap = {};
  buyInv.forEach(inv=>{const k=inv.customer||"Unknown";if(!supMap[k])supMap[k]={owed:0,paid:0};supMap[k].owed+=(+(inv.supplier_amount)||+(inv.total)||0);});
  filtSP.filter(p=>!p.suppliers?.is_shipping).forEach(p=>{const sup=suppliers.find(s=>s.id===p.supplier_id);const k=sup?.name||"Unknown";if(!supMap[k])supMap[k]={owed:0,paid:0};supMap[k].paid+=(+p.amount||0);});
  const supplierBalances = Object.entries(supMap).map(([name,s])=>({name,balance:s.owed-s.paid})).filter(s=>s.balance>0.01).sort((a,b)=>b.balance-a.balance);
  const totalAP = supplierBalances.reduce((s,c)=>s+c.balance,0);

  const shipPayable = Math.max(0, buyInv.reduce((s,i)=>s+(+i.shipment_value||0),0) - filtSP.filter(p=>p.suppliers?.is_shipping).reduce((s,p)=>s+(+p.amount||0),0));

  const totalBsmProfit = buyInv.reduce((s,i)=>s+(+i.partial_bsm||0),0);
  const basemOwed = Math.max(0, totalBsmProfit*0.70 - filtBsm.filter(p=>p.person==="basem").reduce((s,p)=>s+(+p.amount||0),0));
  const chadiOwed = Math.max(0, totalBsmProfit*0.30 - filtBsm.filter(p=>p.person==="chadi").reduce((s,p)=>s+(+p.amount||0),0));
  const bsmPayable = basemOwed + chadiOwed;

  const najibOrabiLocIds = locations.filter(l=>l.name&&l.name.toLowerCase().includes("najib")).map(l=>l.id);
  const noProfit = sellInv.filter(i=>najibOrabiLocIds.includes(i.location_id)).reduce((s,i)=>s+((+i.total||0)-(+i.cogs||0)),0);
  const najibOwed = Math.max(0, noProfit*0.50 - filtNo.filter(p=>p.person==="najib").reduce((s,p)=>s+(+p.amount||0),0));
  const orabiOwed = Math.max(0, noProfit*0.50 - filtNo.filter(p=>p.person==="orabi").reduce((s,p)=>s+(+p.amount||0),0));
  const noPayable = najibOwed + orabiOwed;

  const totalCapital  = filtCap.reduce((s,c)=>s+(+c.amount||0),0);
  const totalRevenue  = sellInv.reduce((s,i)=>s+(+i.total||0),0);
  const totalCOGS     = sellInv.reduce((s,i)=>s+(+i.cogs||0),0);
  const totalExpenses = filtExp.reduce((s,e)=>s+(+e.amount||0),0);
  const netProfit     = totalRevenue - totalCOGS - totalExpenses;

  const totalCurrentAssets = cashBal + usdtBal + bankBal + totalAR + inventoryVal;
  const totalCurrentLiab   = totalAP + shipPayable + bsmPayable + noPayable;
  const bookEquity         = totalCurrentAssets - totalCurrentLiab;
  const totalLiabEquity    = totalCurrentLiab + bookEquity;

  const Row = ({label, amount, sub, indent}) => (
    <div style={{display:"flex",justifyContent:"space-between",padding:"4px 0",paddingLeft:indent?16:0,borderBottom:`0.5px solid ${T.border}22`}}>
      <span style={{fontSize:13,color:sub?T.muted:T.text}}>{label}</span>
      <span style={{fontFamily:T.mono,fontSize:13,color:amount<0?T.red:T.text,fontWeight:sub?400:500}}>
  ...
  const Section = ({title, color, total, totalLabel, children}) => (
    <div style={{marginBottom:20}}>
      <div style={{fontSize:11,fontWeight:700,textTransform:"uppercase",letterSpacing:1,color,borderBottom:`2px solid ${color}`,paddingBottom:6,marginBottom:10}}>{title}</div>
      {children}
      <div style={{display:"flex",justifyContent:"space-between",borderTop:`1px solid ${T.border}`,marginTop:6,paddingTop:8,fontWeight:700}}>
        <span style={{fontSize:13}}>{totalLabel}</span>
        <span style={{fontFamily:T.mono,fontSize:14,color}}>{fmt(total)}</span>
      </div>
    </div>
  );
  ...
  return (
    <div className="page">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20,flexWrap:"wrap",gap:12}}>
        <div>
          <h2 style={{fontSize:20,fontWeight:800,marginBottom:4}}>📊 Balance Sheet</h2>
          <p style={{fontSize:13,color:T.muted}}>Financial position — assets, liabilities & equity</p>
        </div>
        <div style={{display:"flex",gap:10,alignItems:"flex-end",flexWrap:"wrap"}}>
          <div>
            <div style={{fontSize:11,color:T.muted,marginBottom:3}}>AS OF DATE</div>
            <input type="date" value={asOfDate} onChange={e=>setAsOfDate(e.target.value)} style={{padding:"6px 10px",borderRadius:8,border:`1px solid ${T.border}`,background:T.card,color:T.text,fontSize:13}} />
          </div>
          <button onClick={() => {
            const html = `<!DOCTYPE html><html><head><title>Najib Cell — Balance Sheet</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;padding:30px;font-size:12px;}h1{font-size:20px;font-weight:800;margin-bottom:4px;}h2{font-size:13px;color:#555;margin-bottom:20px;}.grid{display:grid;grid-template-columns:1fr 1fr;gap:28px;}.st{font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;border-bottom:2px solid #000;padding-bottom:5px;margin:16px 0 8px;}.row{display:flex;justify-content:space-between;padding:3px 0;font-size:12px;border-bottom:1px solid #f0f0f0;}.row.in{padding-left:14px;color:#666;}.row.sub{font-weight:700;border-top:1px solid #ddd;padding-top:5px;margin-top:3px;}.row.grand{font-weight:800;font-size:14px;border-top:3px double #000;padding-top:8px;margin-top:8px;}.neg{color:#dc2626;}.footer{margin-top:24px;text-align:center;font-size:10px;color:#aaa;border-top:1px solid #eee;padding-top:12px;}</style></head><body><h1>📊 BSM — Balance Sheet</h1><h2>As at ${asOfDate} · In US Dollar ($)</h2><div class="grid"><div><div class="st">ASSETS</div><div class="row"><span>Cash (USD)</span><span class="${cashBal<0?"neg":""}">${cashBal<0?"("+fmt(Math.abs(cashBal))+")":fmt(cashBal)}</span></div><div class="row"><span>Wallet (USDT)</span><span class="${usdtBal<0?"neg":""}">${usdtBal<0?"("+fmt(Math.abs(usdtBal))+")":fmt(usdtBal)}</span></div>${Math.abs(bankBal)>0?`<div class="row"><span>Bank</span><span>${fmt(bankBal)}</span></div>`:""}<div class="st">Accounts Receivable</div>${clientBalances.map(c=>`<div class="row in"><span>${c.name}</span><span>${fmt(c.balance)}</span></div>`).join("")}<div class="row sub"><span>Total Receivables</span><span>${fmt(totalAR)}</span></div><div class="row"><span>Closing Inventory</span><span>${fmt(inventoryVal)}</span></div><div class="row grand"><span>TOTAL ASSETS</span><span>${fmt(totalCurrentAssets)}</span></div></div><div><div class="st">LIABILITIES</div>${supplierBalances.map(s=>`<div class="row in"><span>${s.name}</span><span>${fmt(s.balance)}</span></div>`).join("")}${shipPayable>0?`<div class="row"><span>Shipping Payable</span><span>${fmt(shipPayable)}</span></div>`:""}<div class="row sub"><span>Total Liabilities</span><span>${fmt(totalCurrentLiab)}</span></div><div class="st">CAPITAL & EQUITY</div><div class="row"><span>Owner Capital</span><span>${fmt(totalCapital)}</span></div><div class="row"><span>Net Profit</span><span>${fmt(netProfit)}</span></div><div class="row sub"><span>Total Equity</span><span>${fmt(bookEquity)}</span></div><div class="row grand"><span>TOTAL LIABILITIES & EQUITY</span><span>${fmt(totalLiabEquity)}</span></div></div></div><div class="footer">Printed: ${new Date().toLocaleDateString()} · Najib Cell Management</div><script>window.onload=function(){window.print();window.onafterprint=function(){window.close();}}<\/script></body></html>`;
            const w=window.open("","_blank"); w.document.write(html); w.document.close();
          }} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:8,padding:"8px 16px",fontSize:13,cursor:"pointer",color:T.text}}>🖨️ Print</button>
        </div>
      </div>

      <div style={{background:"transparent",border:`1px solid ${T.green}44`,borderRadius:10,padding:"10px 16px",marginBottom:20,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
        <span style={{fontSize:13,fontWeight:700,color:T.green}}>✅ Balance Sheet — As at {asOfDate}</span>
        <div style={{display:"flex",gap:24,flexWrap:"wrap"}}>
          <span style={{fontSize:12,color:T.muted}}>Assets: <strong style={{color:T.text,fontFamily:T.mono}}>{fmt(totalCurrentAssets)}</strong></span>
          <span style={{fontSize:12,color:T.muted}}>Liabilities + Equity: <strong style={{color:T.text,fontFamily:T.mono}}>{fmt(totalLiabEquity)}</strong></span>
        </div>
      </div>

      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20}}>
        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:20}}>
          <div style={{fontSize:15,fontWeight:800,marginBottom:16,color:T.accent,letterSpacing:2}}>A S S E T S</div>
          <Section title="💵 Current Assets" color="#2563eb" total={totalCurrentAssets} totalLabel="Total Current Assets">
            <Row label="Cash (USD)" amount={cashBal} />
            <Row label="Wallet (USDT)" amount={usdtBal} />
            {Math.abs(bankBal)>0 && <Row label="Bank Transfer" amount={bankBal} />}
            <div style={{fontSize:11,color:T.muted,textTransform:"uppercase",letterSpacing:1,margin:"8px 0 4px"}}>Accounts Receivable</div>
            {clientBalances.length===0 ? <Row label="No outstanding receivables" amount={0} sub /> : clientBalances.map(c=><Row key={c.name} label={c.name} amount={c.balance} indent sub />)}
            <Row label="Total Receivables" amount={totalAR} />
            <Row label="Closing Inventory" amount={inventoryVal} />
          </Section>
          <div style={{display:"flex",justifyContent:"space-between",background:T.accent+"22",borderRadius:8,padding:"10px 14px"}}>
            <span style={{fontWeight:800,fontSize:14}}>TOTAL ASSETS</span>
            <span style={{fontFamily:T.mono,fontWeight:800,fontSize:16,color:T.accent}}>{fmt(totalCurrentAssets)}</span>
          </div>
        </div>

        <div style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:20}}>
          <div style={{fontSize:15,fontWeight:800,marginBottom:16,color:T.red,letterSpacing:2}}>L I A B I L I T I E S &amp; E Q U I T Y</div>
          <Section title="⚠️ Current Liabilities" color={T.red} total={totalCurrentLiab} totalLabel="Total Current Liabilities">
            {supplierBalances.length>0 && <><div style={{fontSize:11,color:T.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:4}}>Accounts Payable</div>{supplierBalances.map(s=><Row key={s.name} label={s.name} amount={s.balance} indent sub />)}<Row label="Total Payables" amount={totalAP} /></>}
            {shipPayable>0 && <Row label="Shipping Payable" amount={shipPayable} />}
            {bsmPayable>0 && <><div style={{fontSize:11,color:T.muted,textTransform:"uppercase",letterSpacing:1,margin:"8px 0 4px"}}>BSM Profit Payable</div>{basemOwed>0&&<Row label="Basem (70%)" amount={basemOwed} indent sub />}{chadiOwed>0&&<Row label="Chadi (30%)" amount={chadiOwed} indent sub />}</>}
            {noPayable>0 && <><div style={{fontSize:11,color:T.muted,textTransform:"uppercase",letterSpacing:1,margin:"8px 0 4px"}}>Najib-Orabi Profit Payable</div>{najibOwed>0&&<Row label="Najib (50%)" amount={najibOwed} indent sub />}{orabiOwed>0&&<Row label="Orabi (50%)" amount={orabiOwed} indent sub />}</>}
          </Section>
          <Section title="💰 Capital & Equity" color={T.green} total={bookEquity} totalLabel="Total Equity (Book Value)">
            <Row label="Owner Capital Contributions" amount={totalCapital} />
            <Row label="Net Profit / Retained Earnings" amount={netProfit} />
          </Section>
          <div style={{display:"flex",justifyContent:"space-between",background:T.green+"22",borderRadius:8,padding:"10px 14px"}}>
            <span style={{fontWeight:800,fontSize:14}}>TOTAL LIABILITIES & EQUITY</span>
            <span style={{fontFamily:T.mono,fontWeight:800,fontSize:16,color:T.green}}>{fmt(totalLiabEquity)}</span>
          </div>
        </div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:12,marginTop:20}}>
        {[
          {label:"Total Assets",     value:fmt(totalCurrentAssets), color:T.accent, icon:"📦"},
          {label:"Total Liabilities",value:fmt(totalCurrentLiab),   color:T.red,    icon:"⚠️"},
          {label:"Total Equity",     value:fmt(bookEquity),         color:T.green,  icon:"💰"},
          {label:"Net Profit",       value:fmt(netProfit),          color:netProfit>=0?T.green:T.red, icon:"📈"},
          {label:"Working Capital",  value:fmt(totalCurrentAssets-totalCurrentLiab), color:T.yellow, icon:"⚡"},
        ].map(c=>(
          <div key={c.label} style={{background:T.card,border:`1px solid ${c.color}44`,borderRadius:12,padding:16}}>
            <div style={{fontSize:11,color:T.muted,textTransform:"uppercase",marginBottom:4}}>{c.icon} {c.label}</div>
            <div style={{fontSize:18,fontWeight:800,color:c.color,fontFamily:T.mono}}>{c.value}</div>
          </div>
        ))}
      </div>
    </div>
...
function TrialBalance({ invoices, expenses, suppliers, clients }) {
  const [fromDate, setFromDate] = useState("");
  const [toDate, setToDate] = useState("");
  const [allData, setAllData] = useState({ sp: [], receipts: [], payments: [] });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const load = async () => {
      setLoading(true);
      const [sp, r, cp, p, cap, exp, bsmPay, noPay] = await Promise.all([
        supabase.from("supplier_payments").select("*, suppliers(name)"),
        supabase.from("receipts").select("*, clients(name)"),
        supabase.from("client_payments").select("*").eq("type", "payment"),
        supabase.from("payments").select("*, suppliers(name)"),
        supabase.from("capital_contributions").select("*"),
        supabase.from("expenses").select("*"),
        supabase.from("bsm_payments").select("*"),
        supabase.from("najib_orabi_payments").select("*"),
      ...
      const allReceipts = [
        ...(r.data||[]).map(x=>({...x, clientName:x.clients?.name||""})),
        ...(cp.data||[]).map(x=>({ ...x, clientName:"", payment_method: x.payment_method||"wallet_usdt" })),
      ];
      setAllData({
        sp: (sp.data||[]).map(x=>({...x, supplierName:x.suppliers?.name||""})),
        receipts: allReceipts,
        payments: (p.data||[]).map(x=>({...x, supplierName:x.suppliers?.name||""})),
        capital: cap.data || [],
        expenses: exp.data || [],
        bsmPayments: bsmPay.data || [],
        najibOrabiPayments: noPay.data || [],
      });
      setLoading(false);
    };
    load();
  ...
  const filtInv = fd(invoices);
  const filtExp = fd(allData.expenses || []);
  const filtReceipts = fd(allData.receipts);
  const filtPayments = fd(allData.payments);
  // Deduplicate supplier_payments vs payments table
  const payKeys = new Set(filtPayments.map(p=>`${p.date}_${+p.amount}_${p.supplierName}`));
  const filtSP = fd(allData.sp).filter(p=>p.type==="payment" && !payKeys.has(`${p.date}_${+p.amount}_${p.supplierName}`));

  const sellInv = filtInv.filter(i=>i.type==="sell");
  const buyInv = filtInv.filter(i=>i.type==="buy");

  // ── REVENUE & EXPENSES (Credit/Debit in Income accounts) ──
  const totalRevenue = sellInv.reduce((s,i)=>s+i.total,0);
  const totalCOGS = sellInv.reduce((s,i)=>s+i.cogs,0);
  const totalExpenses = filtExp.reduce((s,e)=>s+(+e.amount),0);
  const grossProfit = totalRevenue - totalCOGS;
  const netProfit = grossProfit - totalExpenses;

  // ── CASH (Asset — Debit normal) ──
  const cashIn = filtReceipts.filter(r=>r.payment_method==="cash_usd").reduce((s,r)=>s+(+r.amount),0);
  const cashOut = filtPayments.filter(p=>p.payment_method==="cash_usd").reduce((s,p)=>s+(+p.amount),0)
    + filtSP.filter(p=>p.payment_method==="cash_usd").reduce((s,p)=>s+(+p.amount),0)
    + filtExp.filter(e=>e.payment_method==="cash_usd").reduce((s,e)=>s+(+e.amount),0);
  const cashBalance = cashIn - cashOut;

  // ── WALLET USDT (Asset — Debit normal) ──
  const usdtIn = filtReceipts.filter(r=>r.payment_method==="wallet_usdt").reduce((s,r)=>s+(+r.amount),0);
  const usdtOut = filtPayments.filter(p=>p.payment_method==="wallet_usdt").reduce((s,p)=>s+(+p.amount),0)
    + filtSP.filter(p=>p.payment_method==="wallet_usdt").reduce((s,p)=>s+(+p.amount),0)
    + filtExp.filter(e=>e.payment_method==="wallet_usdt").reduce((s,e)=>s+(+e.amount),0);
  const usdtBalance = usdtIn - usdtOut;

  // ── ACCOUNTS RECEIVABLE (Asset — Debit normal) ──
  const totalInvoicedClients = sellInv.reduce((s,i)=>s+i.total,0);
  const totalReceivedClients = filtReceipts.reduce((s,r)=>s+(+r.amount),0);
  const arBalance = Math.max(0, totalInvoicedClients - totalReceivedClients);

  // ── INVENTORY (Asset — Debit normal) ──
  const inventoryIn = buyInv.reduce((s,i)=>s+(i.cogs||0),0);
  const inventoryOut = totalCOGS;
  const inventoryBalance = Math.max(0, inventoryIn - inventoryOut);

  // ── ACCOUNTS PAYABLE (Liability — Credit normal) ──
  const totalOwedSuppliers = buyInv.reduce((s,i)=>s+(i.supplier_amount||i.total),0);
  const totalPaidSuppliers = filtPayments.reduce((s,p)=>s+(+p.amount),0) + filtSP.reduce((s,p)=>s+(+p.amount),0);
  const apBalance = Math.max(0, totalOwedSuppliers - totalPaidSuppliers);

  // ── SHIPPING PAYABLE (Liability — Credit normal) ──
  const totalShipCharged = buyInv.reduce((s,i)=>s+(+i.shipment_value||0),0);
  const shipPaidKeys = new Set(filtPayments.map(p=>`${p.date}_${+p.amount}_${p.supplierName}`));
  const totalShipPaid = fd(allData.sp).filter(p=>p.type==="payment" && suppliers.find(s=>s.is_shipping&&s.id===p.supplier_id)).reduce((s,p)=>s+(+p.amount),0);
  const shipPayable = Math.max(0, totalShipCharged - totalShipPaid);

  // ── CAPITAL / EQUITY (Credit normal) ──
  // Equity = Assets - Liabilities (balancing figure)
  const totalAssets = Math.max(0,cashBalance) + Math.max(0,usdtBalance) + arBalance + inventoryBalance;
  const totalLiabilities = apBalance + shipPayable;
  const equity = totalAssets - totalLiabilities - netProfit; // owners equity excluding retained earnings

  // ── BUILD TRIAL BALANCE ROWS ──
  // Each account: name, debit balance OR credit balance
  // ── CAPITAL (Equity — Credit normal) ──
  const totalCapital = (allData.capital||[]).reduce((s,c)=>s+(+c.amount),0);
  // Capital increases cash/USDT — so it's already in cashBalance/usdtBalance
  // But we need it as an Equity account to balance

  const accounts = [
    // ASSET ACCOUNTS (Debit normal)
    { name: "💵 Cash USD", category: "Asset", debit: Math.max(0, cashBalance), credit: Math.max(0, -cashBalance) },
    { name: "💎 Wallet USDT", category: "Asset", debit: Math.max(0, usdtBalance), credit: Math.max(0, -usdtBalance) },
    { name: "👥 Accounts Receivable", category: "Asset", debit: arBalance, credit: 0 },
    { name: "📦 Inventory", category: "Asset", debit: inventoryBalance, credit: 0 },
    // LIABILITY ACCOUNTS (Credit normal)
    { name: "🏭 Accounts Payable (Suppliers)", category: "Liability", debit: 0, credit: apBalance },
    { name: "🚢 Shipping Payable", category: "Liability", debit: 0, credit: shipPayable },
    // INCOME ACCOUNTS (Credit normal)
    { name: "💰 Sales Revenue", category: "Revenue", debit: 0, credit: totalRevenue },
    // EXPENSE ACCOUNTS (Debit normal)
    { name: "📦 Cost of Goods Sold", category: "Expense", debit: totalCOGS, credit: 0 },
    { name: "💸 Operating Expenses", category: "Expense", debit: totalExpenses, credit: 0 },
    // EQUITY (Credit normal)
    { name: "💰 Owner Capital Contributions", category: "Equity", debit: 0, credit: totalCapital },
    { name: "⚖️ Retained Earnings / Net Profit", category: "Equity", debit: netProfit < 0 ? Math.abs(netProfit) : 0, credit: netProfit > 0 ? netProfit : 0 },
  ];
  ...
  const categoryColor = { Asset: T.accent, Liability: T.red, Revenue: T.green, Expense: T.yellow, Equity: T.green };

  const printTB = () => {
    const rows = accounts.map(a => `<tr>
      <td style="padding:7px 10px">${a.name}</td>
      <td style="text-align:right;padding:7px 10px;color:#cc4400">${a.debit>0?"$"+a.debit.toFixed(2):"—"}</td>
      <td style="text-align:right;padding:7px 10px;color:#006600">${a.credit>0?"$"+a.credit.toFixed(2):"—"}</td>
    </tr>`).join("");
    const period = fromDate||toDate ? `${fromDate||"start"} → ${toDate||"today"}` : "All Time";
    const html = `<!DOCTYPE html><html><head><title>Najib Cell — Trial Balance</title>
    <style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;padding:30px;font-size:13px;}
    h1{font-size:20px;font-weight:800;margin-bottom:4px;}h2{font-size:14px;color:#666;margin-bottom:24px;}
    table{width:100%;border-collapse:collapse;}
    ...
    .balanced{color:green;font-weight:800;} .unbalanced{color:red;font-weight:800;}
    .footer{margin-top:24px;text-align:center;font-size:11px;color:#999;}</style></head>
    <body>
    <h1>📱 Najib Cell — Trial Balance</h1>
    <h2>Period: ${period}</h2>
    <table>
      ...
      <tbody>
        ...
        <tr class="total">
          ...
          <td style="text-align:right;padding:10px;color:#cc4400">$${totalDebits.toFixed(2)}</td>
          <td style="text-align:right;padding:10px;color:#006600">$${totalCredits.toFixed(2)}</td>
        </tr>
        <tr>
          <td colspan="3" style="padding:10px;text-align:center" class="${isBalanced?"balanced":"unbalanced"}">
            ${isBalanced?"✅ BALANCED — Debits equal Credits":"⚠️ DIFFERENCE: $"+Math.abs(diff).toFixed(2)}
          </td>
        </tr>
      </tbody>
    </table>
  ...
  return (
    <div className="page">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        <div>
          <h2 style={{ fontSize:28, fontWeight:800 }}>📋 Trial Balance</h2>
          <p style={{ fontSize:13, marginTop:4, fontWeight:700, color: isBalanced?T.green:T.yellow }}>
            {isBalanced ? "✅ Balanced — Debits = Credits" : `⚠️ Difference: ${fmt(Math.abs(diff))}`}
          </p>
        </div>
        <button onClick={printTB} style={{ background:T.green+"22", border:`1px solid ${T.green}44`, borderRadius:8, padding:"8px 18px", color:T.green, fontSize:13, fontWeight:700, cursor:"pointer" }}>🖨️ Print</button>
      </div>
        ...
        <div style={{ background:T.card, border:`1px solid ${T.border}`, borderRadius:12, overflow:"hidden" }}>
          <table>
            <thead>
              <tr>
                <th>Account</th>
                <th style={{ textAlign:"right", color:T.red }}>Debit ($)</th>
                <th style={{ textAlign:"right", color:T.green }}>Credit ($)</th>
              </tr>
            </thead>
            <tbody>
              {/* Group by category */}
              {["Asset","Liability","Revenue","Expense","Equity"].map(cat => (
                <>
                  <tr key={`cat-${cat}`} style={{ background:T.surface }}>
                    <td colSpan={3} style={{ fontWeight:800, fontSize:11, color:categoryColor[cat], textTransform:"uppercase", letterSpacing:2, padding:"8px 16px" }}>{cat} Accounts</td>
                  </tr>
                  {accounts.filter(a=>a.category===cat).map((a, i) => (
                    <tr key={i}>
                      <td style={{ padding:"9px 16px 9px 28px", fontSize:13 }}>{a.name}</td>
                      <td style={{ textAlign:"right", fontFamily:T.mono, color:T.red, fontWeight:600, padding:"9px 16px" }}>{a.debit>0?fmt(a.debit):"—"}</td>
                      <td style={{ textAlign:"right", fontFamily:T.mono, color:T.green, fontWeight:600, padding:"9px 16px" }}>{a.credit>0?fmt(a.credit):"—"}</td>
                    </tr>
                  ))}
                </>
              ))}
              {/* TOTALS ROW */}
              <tr style={{ background:T.surface, borderTop:`3px solid ${isBalanced?T.green:T.red}` }}>
                <td style={{ fontWeight:800, fontSize:15, padding:"14px 16px" }}>TOTALS</td>
                <td style={{ textAlign:"right", fontFamily:T.mono, fontWeight:800, fontSize:16, color:T.red, padding:"14px 16px" }}>{fmt(totalDebits)}</td>
                <td style={{ textAlign:"right", fontFamily:T.mono, fontWeight:800, fontSize:16, color:T.green, padding:"14px 16px" }}>{fmt(totalCredits)}</td>
              </tr>
              {/* BALANCE CHECK */}
              <tr style={{ background: isBalanced?T.green+"11":T.yellow+"11" }}>
                <td colSpan={3} style={{ textAlign:"center", padding:"12px", fontWeight:800, fontSize:14, color:isBalanced?T.green:T.yellow }}>
                  {isBalanced ? "✅ BALANCED — Total Debits = Total Credits" : `⚠️ DIFFERENCE OF ${fmt(Math.abs(diff))} — Check entries`}
                </td>
              </tr>
            </tbody>
          </table>
...
function TechPage() {
  ...
  const emptyForm = {
    ...
    customer_name: "", phone: "", model: "", problem: "",
    technician: "", parts_cost: "", repair_cost: "",
    status: "pending", payment_status: "unpaid",
    payment_method: "cash_usd", notes: "", warranty_days: "30"
  };
  const [form, setForm] = useState(emptyForm);

  const load = async () => {
    setLoading(true);
    const { data } = await supabase.from("tech_tickets").select("*").order("date", { ascending: false });
    setTickets(data || []);
    setLoading(false);
  };
  ...
  const save = async () => {
    if (!form.customer_name || !form.model) return;
    setSaving(true);
    if (editTicket) {
      const { error } = await supabase.from("tech_tickets").update({
        date: form.date, customer_name: form.customer_name, phone: form.phone,
        model: form.model, problem: form.problem, technician: form.technician,
        parts_cost: +form.parts_cost||0, repair_cost: +form.repair_cost||0,
        status: form.status, payment_status: form.payment_status,
        payment_method: form.payment_method, notes: form.notes,
        warranty_days: +form.warranty_days||30,
      }).eq("id", editTicket.id);
  ...
  const del = async (id) => {
    ...
    await supabase.from("tech_tickets").delete().eq("id", id);
    await load();
  };

  const openEdit = (t) => {
    setForm({ ...t, parts_cost: t.parts_cost||"", repair_cost: t.repair_cost||"", warranty_days: t.warranty_days||30 });
    setEditTicket(t);
    setShowAdd(true);
  };

  const printTicket = (t) => {
    const profit = (t.repair_cost||0) - (t.parts_cost||0);
    const html = `<!DOCTYPE html><html><head><title>Najib Cell — Tech Ticket #${t.id}</title>
    <style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;padding:40px;font-size:13px;}
    h1{font-size:22px;font-weight:800;margin-bottom:4px;}
    .subtitle{color:#666;margin-bottom:24px;font-size:13px;}
    .grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:24px;padding:16px;background:#f8f9fa;border-radius:8px;}
    .label{font-size:11px;color:#999;text-transform:uppercase;letter-spacing:1px;margin-bottom:3px;}
    .value{font-size:14px;font-weight:600;}
    .problem-box{padding:14px;background:#fff3cd;border-radius:8px;margin-bottom:20px;border:1px solid #ffc107;}
    .financials{display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:20px;}
    .fin-card{padding:12px;border-radius:8px;text-align:center;}
    ...
    <div class="subtitle">📱 Najib Cell Management • Ticket #${t.id}</div>
    <div class="grid">
      <div><div class="label">Customer</div><div class="value">${t.customer_name}</div></div>
      <div><div class="label">Phone</div><div class="value">${t.phone||"—"}</div></div>
      <div><div class="label">Model</div><div class="value">${t.model}</div></div>
      <div><div class="label">Date</div><div class="value">${t.date}</div></div>
      <div><div class="label">Technician</div><div class="value">${t.technician||"—"}</div></div>
      <div><div class="label">Warranty</div><div class="value">${t.warranty_days||30} days</div></div>
    </div>
    <div class="problem-box">
      <div class="label" style="margin-bottom:6px">Problem Description</div>
      <div style="font-size:14px">${t.problem||"—"}</div>
    </div>
    <div class="financials">
      <div class="fin-card" style="background:#fee2e2">
        <div class="label">Parts Cost</div>
        <div style="font-size:18px;font-weight:800;color:#dc2626">$${(+t.parts_cost||0).toFixed(2)}</div>
      </div>
      <div class="fin-card" style="background:#d1fae5">
        <div class="label">Repair Charge</div>
        <div style="font-size:18px;font-weight:800;color:#059669">$${(+t.repair_cost||0).toFixed(2)}</div>
      </div>
      <div class="fin-card" style="background:#dbeafe">
        <div class="label">Profit</div>
        <div style="font-size:18px;font-weight:800;color:#2563eb">$${profit.toFixed(2)}</div>
      </div>
    </div>
    ${t.notes?`<div style="padding:12px;background:#f1f5f9;border-radius:8px;margin-bottom:16px"><div class="label" style="margin-bottom:4px">Notes</div><div>${t.notes}</div></div>`:""}
    <div style="display:flex;gap:12px">
      <span class="status-badge" style="background:${t.status==="fixed"?"#d1fae5;color:#065f46":t.status==="in_progress"?"#fef3c7;color:#92400e":"#fee2e2;color:#991b1b"}">${t.status?.replace("_"," ").toUpperCase()}</span>
      <span class="status-badge" style="background:${t.payment_status==="paid"?"#d1fae5;color:#065f46":"#fef3c7;color:#92400e"}">${t.payment_status?.toUpperCase()}</span>
    </div>
    <div class="footer">Printed: ${new Date().toLocaleDateString()} • Najib Cell Management</div>
    <script>window.onload=function(){window.print();window.onafterprint=function(){window.close();}}</script>
    </body></html>`;
  ...
  const totalRevenue = filtered.reduce((s,t)=>s+(+t.repair_cost||0),0);
  const totalCosts = filtered.reduce((s,t)=>s+(+t.parts_cost||0),0);
  const totalProfit = totalRevenue - totalCosts;
  const unpaidCount = filtered.filter(t=>t.payment_status==="unpaid").length;
  const unpaidAmount = filtered.filter(t=>t.payment_status==="unpaid").reduce((s,t)=>s+(+t.repair_cost||0),0);

  const statusColor = { pending:"#ef4444", in_progress:"#f59e0b", fixed:"#22c55e", returned:"#6b7280" };
  const statusLabel = { pending:"⏳ Pending", in_progress:"🔧 In Progress", fixed:"✅ Fixed", returned:"📦 Returned" };
  const payColor = { paid:T.green, unpaid:T.yellow };

  const STATUSES = ["pending","in_progress","fixed","returned"];

  return (
    <div className="page">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20, flexWrap:"wrap", gap:12 }}>
        <div>
          <h2 style={{ fontSize:28, fontWeight:800 }}>🔧 Technical Assistance</h2>
          <p style={{ color:T.muted, fontSize:13, marginTop:4 }}>Track phone repairs and technical service</p>
        </div>
        <Btn onClick={()=>{ setShowAdd(!showAdd); setEditTicket(null); setForm(emptyForm); }}>+ New Ticket</Btn>
      </div>

      {/* Summary Cards */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:14, marginBottom:24 }}>
        {[
          { label:"Total Tickets", val:filtered.length, unit:"tickets", color:T.accent },
          { label:"Revenue", val:fmt(totalRevenue), color:T.green },
          { label:"Parts Cost", val:fmt(totalCosts), color:T.red },
          { label:"Profit", val:fmt(totalProfit), color:totalProfit>=0?T.green:T.red },
          { label:"Unpaid", val:fmt(unpaidAmount), unit:`${unpaidCount} tickets`, color:T.yellow },
        ].map((c,i) => (
          <div key={i} style={{ background:T.card, border:`1px solid ${c.color}44`, borderRadius:12, padding:16 }}>
            <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:6 }}>{c.label}</div>
            <div style={{ fontSize:20, fontWeight:800, color:c.color, fontFamily:T.mono }}>{c.val}</div>
            {c.unit && <div style={{ fontSize:11, color:T.muted, marginTop:2 }}>{c.unit}</div>}
          </div>
        ))}
      </div>
      ...
      {showAdd && (
        <div style={{ background:T.card, border:`1px solid ${T.accent}44`, borderRadius:12, padding:24, marginBottom:24 }}>
          <h3 style={{ fontSize:14, fontWeight:700, color:T.accent, marginBottom:16 }}>{editTicket?"✏️ Edit Ticket":"🔧 New Repair Ticket"}</h3>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:12, marginBottom:16 }}>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>DATE</div><input type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})} /></div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>CUSTOMER NAME</div><input value={form.customer_name} onChange={e=>setForm({...form,customer_name:e.target.value})} placeholder="Customer name" /></div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>PHONE NUMBER</div><input value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})} placeholder="Phone number" /></div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>PHONE MODEL</div><input value={form.model} onChange={e=>setForm({...form,model:e.target.value})} placeholder="e.g. iPhone 15, Samsung S24" /></div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>TECHNICIAN</div><input value={form.technician} onChange={e=>setForm({...form,technician:e.target.value})} placeholder="Technician name" /></div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>WARRANTY (days)</div><input type="number" value={form.warranty_days} onChange={e=>setForm({...form,warranty_days:e.target.value})} placeholder="30" min="0" /></div>
          </div>
          <div style={{ marginBottom:12 }}>
            <div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>PROBLEM DESCRIPTION</div>
            <textarea value={form.problem} onChange={e=>setForm({...form,problem:e.target.value})} placeholder="Describe the problem..." rows={3} style={{ width:"100%", padding:"8px 12px", background:T.surface, border:`1px solid ${T.border}`, borderRadius:8, color:T.text, fontSize:13, resize:"vertical" }} />
          </div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(160px,1fr))", gap:12, marginBottom:16 }}>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>PARTS COST ($)</div><input type="number" value={form.parts_cost} onChange={e=>setForm({...form,parts_cost:e.target.value})} placeholder="0.00" min="0" /></div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>REPAIR CHARGE ($)</div><input type="number" value={form.repair_cost} onChange={e=>setForm({...form,repair_cost:e.target.value})} placeholder="0.00" min="0" /></div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>STATUS</div>
            ...
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>PAYMENT STATUS</div>
          ...
          <div style={{ marginBottom:16 }}>
            <div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>NOTES</div>
            <input value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Additional notes..." style={{ width:"100%" }} />
          </div>
          {/* Profit preview */}
          {(+form.repair_cost > 0 || +form.parts_cost > 0) && (
            <div style={{ display:"flex", gap:16, marginBottom:16, background:T.surface, borderRadius:8, padding:12, fontFamily:T.mono, fontSize:13 }}>
              <span style={{ color:T.red }}>Parts: {fmt(+form.parts_cost||0)}</span>
              <span style={{ color:T.green }}>Charge: {fmt(+form.repair_cost||0)}</span>
              <span style={{ color:(+form.repair_cost||0)-(+form.parts_cost||0)>=0?T.green:T.red, fontWeight:700 }}>
                Profit: {fmt((+form.repair_cost||0)-(+form.parts_cost||0))}
              </span>
            </div>
          )}
          <div style={{ display:"flex", gap:10 }}>
            <Btn onClick={save} loading={saving}>{editTicket?"Save Changes":"Create Ticket"}</Btn>
            <Btn outline onClick={()=>{ setShowAdd(false); setEditTicket(null); setForm(emptyForm); }}>Cancel</Btn>
          </div>
        </div>
      )}
      ...
      <div style={{ display:"flex", gap:10, marginBottom:16, flexWrap:"wrap", alignItems:"center" }}>
        <div style={{ display:"flex", gap:6 }}>
          {["all","pending","in_progress","fixed","returned"].map(s => (
            <button key={s} onClick={()=>setFilterStatus(s)}
              style={{ padding:"5px 12px", borderRadius:8, border:`1px solid ${filterStatus===s?(statusColor[s]||T.accent):T.border}`, background:filterStatus===s?(statusColor[s]||T.accent)+"22":"transparent", color:filterStatus===s?(statusColor[s]||T.accent):T.muted, fontSize:12, fontWeight:filterStatus===s?700:400, cursor:"pointer" }}>
        ...
        <div style={{ display:"flex", gap:8, alignItems:"center", marginLeft:"auto" }}>
          ...
          <input type="date" value={toDate} onChange={e=>setToDate(e.target.value)} style={{ width:"auto" }} />
          <button onClick={()=>{setFromDate("");setToDate("");}} style={{ background:"transparent", border:`1px solid ${T.border}`, borderRadius:6, padding:"4px 8px", color:T.muted, fontSize:11, cursor:"pointer" }}>Clear</button>
        </div>
      </div>
      ...
      {loading ? <Loader /> : (
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {filtered.length === 0 && <div style={{ textAlign:"center", padding:40, color:T.muted }}>No tickets found</div>}
          {filtered.map(t => {
            const profit = (t.repair_cost||0) - (t.parts_cost||0);
            return (
              <div key={t.id} style={{ background:T.card, border:`1px solid ${statusColor[t.status]||T.border}44`, borderRadius:12, overflow:"hidden" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", padding:"14px 20px", flexWrap:"wrap", gap:10 }}>
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:10, flexWrap:"wrap", marginBottom:6 }}>
                      <span style={{ fontWeight:800, fontSize:15 }}>{t.customer_name}</span>
                      <Badge color={statusColor[t.status]}>{statusLabel[t.status]}</Badge>
                      <Badge color={payColor[t.payment_status]}>{t.payment_status==="paid"?"✅ Paid":"⏳ Unpaid"}</Badge>
                    </div>
                    <div style={{ fontSize:13, color:T.accent, fontWeight:600, marginBottom:4 }}>📱 {t.model}</div>
                    <div style={{ fontSize:12, color:T.muted, marginBottom:t.problem?4:0 }}>{t.date} {t.technician?`• 👨‍🔧 ${t.technician}`:""} {t.warranty_days?`• 🛡️ ${t.warranty_days}d warranty`:""}</div>
                    {t.problem && <div style={{ fontSize:12, color:T.text, marginTop:4, padding:"6px 10px", background:T.surface, borderRadius:6 }}>🔍 {t.problem}</div>}
                    {t.notes && <div style={{ fontSize:11, color:T.muted, marginTop:4 }}>📝 {t.notes}</div>}
                  </div>
                  <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:6 }}>
                    <div style={{ display:"flex", gap:16, fontFamily:T.mono, fontSize:13 }}>
                      <span style={{ color:T.red }}>Parts: {fmt(t.parts_cost||0)}</span>
                      <span style={{ color:T.green }}>Charge: {fmt(t.repair_cost||0)}</span>
                      <span style={{ color:profit>=0?T.green:T.red, fontWeight:700 }}>Profit: {fmt(profit)}</span>
                    </div>
                    <div style={{ display:"flex", gap:8 }}>
                      <button onClick={()=>openEdit(t)} style={{ background:T.accent+"22", border:`1px solid ${T.accent}44`, borderRadius:6, padding:"4px 10px", color:T.accent, fontSize:12, cursor:"pointer" }}>✏️</button>
                      <button onClick={()=>printTicket(t)} style={{ background:T.green+"22", border:`1px solid ${T.green}44`, borderRadius:6, padding:"4px 10px", color:T.green, fontSize:12, cursor:"pointer" }}>🖨️</button>
                      <button onClick={()=>del(t.id)} style={{ background:T.red+"22", border:`1px solid ${T.red}44`, borderRadius:6, padding:"4px 10px", color:T.red, fontSize:12, cursor:"pointer" }}>🗑️</button>
                    </div>
                  </div>
                </div>
              </div>
...
// ─── BSM Balance ──────────────────────────────────────────────────────────────
function BSMBalance({ invoices, onRefresh }) {
  const PARTNERS = [
    { key: "basem", label: "Basem", pct: 0.70, color: "#6366f1", icon: "👤" },
    { key: "chadi", label: "Chadi", pct: 0.30, color: "#f59e0b", icon: "👤" },
  ];
  const [tab, setTab]           = useState("basem");
  const [payments, setPayments] = useState([]);
  const [loading, setLoading]   = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [form, setForm]         = useState({ date: new Date().toISOString().slice(0,10), amount: "", notes: "", payment_method: "cash_usd" });
  const [saving, setSaving]     = useState(false);
  const [filterFrom, setFilterFrom] = useState("");
  const [filterTo, setFilterTo]     = useState("");

  const bsmInvoices = invoices.filter(i => i.type === "buy" && (+i.partial_bsm || 0) > 0);
  const totalProfit = bsmInvoices.reduce((s, i) => s + (+i.partial_bsm || 0), 0);
  const loadPayments = async () => { setLoading(true); const { data } = await supabase.from("bsm_payments").select("*").order("date", { ascending: true }); setPayments(data || []); setLoading(false); };
  useEffect(() => { loadPayments(); }, []);

  const pmLabel = { cash_usd: "💵 Cash USD", wallet_usdt: "💎 USDT", bank_transfer: "🏦 Bank" };
  const partner = PARTNERS.find(p => p.key === tab);
  const partnerShare = +(totalProfit * partner.pct).toFixed(2);
  const partnerPayments = payments.filter(p => p.person === tab);
  const totalPaid = partnerPayments.reduce((s, p) => s + (+p.amount), 0);
  const balance = partnerShare - totalPaid;

  const ledgerRows = [
    ...bsmInvoices.map(inv => ({ date: inv.date, type: "debit", description: "Invoice " + inv.id, notes: inv.customer, amount: +(inv.partial_bsm * partner.pct).toFixed(2), id: "inv-" + inv.id })),
    ...partnerPayments.map(p => ({ date: p.date, type: "credit", description: "Payment to " + partner.label, notes: p.notes||"", amount: +p.amount, id: "pay-"+p.id, payId: p.id, method: p.payment_method })),
  ].sort((a,b) => a.date.localeCompare(b.date));

  const filtered = ledgerRows.filter(r => { if (filterFrom && r.date < filterFrom) return false; if (filterTo && r.date > filterTo) return false; return true; });
  let running = 0;
  const rows = filtered.map(r => { if (r.type==="debit") running+=r.amount; else running-=r.amount; return {...r, running}; });

  const save = async () => {
    if (!form.amount || +form.amount <= 0) return alert("Enter a valid amount");
    setSaving(true);
    await supabase.from("bsm_payments").insert({ date: form.date, amount: +form.amount, notes: form.notes, type: "payment", payment_method: form.payment_method, person: tab });
    setForm({ date: new Date().toISOString().slice(0,10), amount: "", notes: "", payment_method: "cash_usd" });
    setShowForm(false); await loadPayments(); setSaving(false);
  };
  const deletePayment = async (payId) => { if (!window.confirm("Delete?")) return; await supabase.from("bsm_payments").delete().eq("id", payId); await loadPayments(); };
  if (loading) return <Loader />;

  return (
    <div className="page">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20, flexWrap:"wrap", gap:12 }}>
        <h2 style={{ fontSize:18, fontWeight:800 }}>🏦 BSM Profit Balance</h2>
        <button onClick={() => setShowForm(!showForm)} style={{ background:partner.color, border:"none", borderRadius:8, padding:"8px 16px", fontSize:13, fontWeight:700, color:"#fff", cursor:"pointer" }}>+ Record Payment</button>
      </div>
      <div style={{ background:T.card, border:"1px solid "+T.border, borderRadius:12, padding:"14px 20px", marginBottom:20, display:"flex", gap:32, flexWrap:"wrap", alignItems:"center" }}>
        <div>
          <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:4 }}>📑 Total BSM Profit</div>
          <div style={{ fontSize:22, fontWeight:800, color:T.yellow, fontFamily:T.mono }}>{fmt(totalProfit)}</div>
          <div style={{ fontSize:11, color:T.muted }}>{bsmInvoices.length} invoices</div>
        </div>
        {PARTNERS.map(p => { const share=+(totalProfit*p.pct).toFixed(2); const paid=payments.filter(x=>x.person===p.key).reduce((s,x)=>s+(+x.amount),0); const bal=share-paid; return (
          <div key={p.key} style={{ borderLeft:"3px solid "+p.color, paddingLeft:16 }}>
            <div style={{ fontSize:11, color:T.muted, marginBottom:2 }}>{p.icon} {p.label} ({(p.pct*100)}%)</div>
            <div style={{ fontSize:16, fontWeight:800, color:p.color, fontFamily:T.mono }}>{fmt(share)}</div>
            <div style={{ fontSize:11, color:bal>0?T.red:T.green, marginTop:2 }}>{bal>0?"Owes: "+fmt(bal):"✅ Settled"}</div>
          </div>
        ); })}
      </div>
      <div style={{ display:"flex", gap:8, marginBottom:20 }}>
        {PARTNERS.map(p => (
          <button key={p.key} onClick={() => { setTab(p.key); setShowForm(false); }}
            style={{ flex:1, padding:"10px 0", borderRadius:10, border:"2px solid "+(tab===p.key?p.color:T.border), background:tab===p.key?p.color+"22":"transparent", color:tab===p.key?p.color:T.muted, fontWeight:700, fontSize:14, cursor:"pointer" }}>
            {p.icon} {p.label} <span style={{ fontSize:11, opacity:0.8 }}>({(p.pct*100)}%)</span>
          </button>
        ))}
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:14, marginBottom:20 }}>
        {[{label:"Share Owed",value:fmt(partnerShare),color:T.yellow,icon:"📑",sub:(partner.pct*100)+"% of total"},{label:"Total Paid",value:fmt(totalPaid),color:T.green,icon:"✅",sub:partnerPayments.length+" payments"},{label:"Balance",value:fmt(balance),color:balance>0?T.red:T.green,icon:"💳",sub:balance>0?"Outstanding":"All clear"}].map(c => (
          <div key={c.label} style={{ background:T.card, border:"1px solid "+c.color+"44", borderRadius:12, padding:18 }}>
            <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:6 }}>{c.icon} {c.label}</div>
            <div style={{ fontSize:22, fontWeight:800, color:c.color, fontFamily:T.mono }}>{c.value}</div>
            <div style={{ fontSize:11, color:T.muted, marginTop:4 }}>{c.sub}</div>
          </div>
        ))}
      </div>
      {showForm && (
        <div style={{ background:T.card, border:"2px solid "+partner.color+"44", borderRadius:12, padding:20, marginBottom:20 }}>
          <h3 style={{ fontSize:13, fontWeight:700, marginBottom:14, textTransform:"uppercase", color:partner.color }}>Record Payment → {partner.label}</h3>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:12, marginBottom:14 }}>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>DATE</div><input type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})} /></div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>AMOUNT</div><input type="number" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})} placeholder="0.00" />{balance>0&&<div style={{ fontSize:10, color:T.muted, marginTop:2 }}>Balance: {fmt(balance)}</div>}</div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>METHOD</div><select value={form.payment_method} onChange={e=>setForm({...form,payment_method:e.target.value})}><option value="cash_usd">💵 Cash USD</option><option value="wallet_usdt">💎 USDT</option><option value="bank_transfer">🏦 Bank</option></select></div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>NOTES</div><input value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Optional" /></div>
          </div>
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={save} disabled={saving} style={{ background:partner.color, border:"none", borderRadius:8, padding:"9px 20px", fontSize:13, fontWeight:700, color:"#fff", cursor:"pointer" }}>{saving?"Saving...":"💾 Save"}</button>
            <button onClick={()=>setShowForm(false)} style={{ background:"transparent", border:"1px solid "+T.border, borderRadius:8, padding:"9px 16px", fontSize:13, color:T.muted, cursor:"pointer" }}>Cancel</button>
          </div>
        </div>
      )}
      <div style={{ display:"flex", gap:12, alignItems:"center", marginBottom:16, flexWrap:"wrap" }}>
        <span style={{ fontSize:12, color:T.muted }}>📅 From:</span>
        <input type="date" value={filterFrom} onChange={e=>setFilterFrom(e.target.value)} style={{ padding:"4px 8px", fontSize:12, borderRadius:6, border:"1px solid "+T.border, background:T.card, color:T.text }} />
        <span style={{ fontSize:12, color:T.muted }}>To:</span>
        <input type="date" value={filterTo} onChange={e=>setFilterTo(e.target.value)} style={{ padding:"4px 8px", fontSize:12, borderRadius:6, border:"1px solid "+T.border, background:T.card, color:T.text }} />
        {(filterFrom||filterTo)&&<button onClick={()=>{setFilterFrom("");setFilterTo("");}} style={{ background:"transparent", border:"1px solid "+T.border, borderRadius:6, padding:"4px 10px", fontSize:12, color:T.muted, cursor:"pointer" }}>Clear</button>}
      </div>
      <div style={{ background:T.card, border:"1px solid "+T.border, borderRadius:12, overflow:"hidden" }}>
        <div style={{ padding:"12px 20px", borderBottom:"1px solid "+T.border, background:partner.color+"11" }}>
          <span style={{ fontWeight:700, color:partner.color }}>{partner.icon} {partner.label} Ledger</span>
          <span style={{ fontSize:11, color:T.muted, marginLeft:8 }}>({(partner.pct*100)}% share)</span>
        </div>
        <table>
          <thead><tr><th>Date</th><th>Description</th><th>Notes</th><th style={{ color:T.red }}>Debit</th><th style={{ color:T.green }}>Credit</th><th>Balance</th><th>Actions</th></tr></thead>
          <tbody>
            {rows.length===0&&<tr><td colSpan={7} style={{ textAlign:"center", color:T.muted, padding:32 }}>No transactions yet</td></tr>}
            {rows.map(r=>(
              <tr key={r.id}>
                <td style={{ fontSize:12, color:T.muted }}>{r.date}</td>
                <td style={{ fontSize:13, fontWeight:600 }}>{r.description}</td>
                <td style={{ fontSize:12, color:T.muted }}>{r.notes}{r.method?<span style={{ marginLeft:6 }}><Badge color={r.method==="cash_usd"?T.green:r.method==="wallet_usdt"?T.accent:T.yellow}>{pmLabel[r.method]}</Badge></span>:""}</td>
                <td style={{ fontFamily:T.mono, color:T.red, fontWeight:700 }}>{r.type==="debit"?fmt(r.amount):"—"}</td>
                <td style={{ fontFamily:T.mono, color:T.green, fontWeight:700 }}>{r.type==="credit"?fmt(r.amount):"—"}</td>
                <td style={{ fontFamily:T.mono, fontWeight:700, color:r.running>0?T.red:T.green }}>{fmt(Math.abs(r.running))} {r.running>0?"DR":"CR"}</td>
                <td>{r.type==="credit"&&<button onClick={()=>deletePayment(r.payId)} style={{ background:"transparent", border:"1px solid "+T.red+"44", borderRadius:4, padding:"2px 8px", color:T.red, fontSize:11, cursor:"pointer" }}>🗑️</button>}</td>
              </tr>
            ))}
          </tbody>
        </table>
      ...
      {bsmInvoices.length>0&&(
        <div style={{ marginTop:20, background:T.card, border:"1px solid "+T.border, borderRadius:12, overflow:"hidden" }}>
          <div style={{ padding:"14px 20px", borderBottom:"1px solid "+T.border }}><h3 style={{ fontSize:13, fontWeight:700, textTransform:"uppercase", color:T.accent }}>📑 BSM Profit by Invoice</h3></div>
          <table>
            <thead><tr><th>Invoice</th><th>Date</th><th>Supplier</th><th style={{ color:T.green }}>Total BSM</th><th style={{ color:"#6366f1" }}>Basem (70%)</th><th style={{ color:"#f59e0b" }}>Chadi (30%)</th></tr></thead>
            <tbody>
              {bsmInvoices.map(inv=>(
                <tr key={inv.id}>
                  <td style={{ fontFamily:T.mono, color:T.accent, fontSize:12 }}>{inv.id}</td>
                  ...
                  <td style={{ fontWeight:600 }}>{inv.customer}</td>
                  <td style={{ fontFamily:T.mono, fontWeight:700, color:T.green }}>{fmt(+inv.partial_bsm)}</td>
                  <td style={{ fontFamily:T.mono, color:"#6366f1" }}>{fmt(+(inv.partial_bsm*0.70).toFixed(2))}</td>
                  <td style={{ fontFamily:T.mono, color:"#f59e0b" }}>{fmt(+(inv.partial_bsm*0.30).toFixed(2))}</td>
                </tr>
              ))}
              <tr style={{ borderTop:"2px solid "+T.border }}>
                <td colSpan={3} style={{ fontWeight:700, textAlign:"right", paddingRight:16 }}>Total:</td>
                <td style={{ fontFamily:T.mono, fontWeight:800, color:T.green }}>{fmt(totalProfit)}</td>
                <td style={{ fontFamily:T.mono, fontWeight:800, color:"#6366f1" }}>{fmt(+(totalProfit*0.70).toFixed(2))}</td>
                <td style={{ fontFamily:T.mono, fontWeight:800, color:"#f59e0b" }}>{fmt(+(totalProfit*0.30).toFixed(2))}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
// ─── Najib-Orabi Balance ──────────────────────────────────────────────────────
function NajibOrabiBalance({ invoices, locations, onRefresh }) {
  const PARTNERS = [
    { key: "najib", label: "Najib", pct: 0.50, color: "#a855f7", icon: "👤" },
    { key: "orabi", label: "Orabi", pct: 0.50, color: "#00e5ff", icon: "👤" },
  ];

  const [tab, setTab]               = useState("najib");
  const [payments, setPayments]     = useState([]);
  const [loading, setLoading]       = useState(true);
  const [showForm, setShowForm]     = useState(false);
  const [form, setForm]             = useState({ date: new Date().toISOString().slice(0,10), amount: "", notes: "", payment_method: "cash_usd" });
  ...
  // Find Najib-Orabi location(s) — match by name containing "Najib"
  const najibOrabiLocIds = locations.filter(l => l.name && l.name.toLowerCase().includes("najib")).map(l => l.id);

  // Only sell invoices from Najib-Orabi location
  const noInvoices = invoices.filter(i =>
    i.type === "sell" &&
    najibOrabiLocIds.includes(i.location_id) &&
    (!filterFrom || i.date >= filterFrom) &&
    (!filterTo   || i.date <= filterTo)
  );

  const totalRevenue = noInvoices.reduce((s, i) => s + (+i.total || 0), 0);
  const totalCogs    = noInvoices.reduce((s, i) => s + (+i.cogs  || 0), 0);
  const totalProfit  = totalRevenue - totalCogs;

  const loadPayments = async () => {
    setLoading(true);
    const { data } = await supabase.from("najib_orabi_payments").select("*").order("date", { ascending: true });
    setPayments(data || []);
    setLoading(false);
  };
  ...
  // Ledger: one debit row per invoice (partner's share of profit) + payment credits
  const ledgerRows = [
    ...noInvoices.map(inv => {
      const invProfit = (+inv.total || 0) - (+inv.cogs || 0);
      const share = +(invProfit * partner.pct).toFixed(2);
      return { date: inv.date, type: "debit", description: "Invoice " + inv.id, notes: inv.customer, amount: share, id: "inv-" + inv.id };
    }),
    ...partnerPayments.map(p => ({
      date: p.date, type: "credit",
      description: "Payment to " + partner.label,
      notes: p.notes || "", amount: +p.amount,
      id: "pay-" + p.id, payId: p.id, method: p.payment_method,
    })),
  ].sort((a, b) => a.date.localeCompare(b.date));

  let running = 0;
  const rows = ledgerRows.map(r => {
    if (r.type === "debit") running += r.amount; else running -= r.amount;
    return { ...r, running };
  });
  ...
  const save = async () => {
    if (!form.amount || +form.amount <= 0) return alert("Enter a valid amount");
    setSaving(true);
    await supabase.from("najib_orabi_payments").insert({
      date: form.date, amount: +form.amount,
      notes: form.notes, type: "payment",
  ...
  const deletePayment = async (payId) => {
    ...
    await supabase.from("najib_orabi_payments").delete().eq("id", payId);
    await loadPayments();
  };

  const printLedger = () => {
    const rowsHtml = rows.map(r =>
      "<tr><td>" + r.date + "</td><td>" + r.description + "</td><td>" + r.notes + "</td>" +
      "<td style=\"color:#dc2626\">" + (r.type==="debit"?fmt(r.amount):"—") + "</td>" +
      "<td style=\"color:#16a34a\">" + (r.type==="credit"?fmt(r.amount):"—") + "</td>" +
      "<td style=\"font-weight:700\">" + fmt(Math.abs(r.running)) + " " + (r.running>0?"DR":"CR") + "</td></tr>"
    ).join("");
    const html = "<!DOCTYPE html><html><head><title>Najib-Orabi — " + partner.label + "</title>" +
      "<style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;padding:30px;font-size:12px;}" +
      "h1{font-size:18px;font-weight:800;margin-bottom:4px;}h2{font-size:14px;color:#666;margin-bottom:20px;}" +
      "table{width:100%;border-collapse:collapse;}th{background:#000;color:#fff;padding:8px;text-align:left;}" +
      "td{padding:7px 8px;border-bottom:1px solid #eee;}.footer{margin-top:20px;text-align:center;font-size:10px;color:#999;}</style></head><body>" +
      "<h1>Najib-Orabi Profit — " + partner.label + " (50%)</h1>" +
      "<h2>Total Profit: " + fmt(totalProfit) + " | Share: " + fmt(partnerShare) + " | Paid: " + fmt(totalPaid) + " | Balance: " + fmt(balance) + "</h2>" +
      "<table><thead><tr><th>Date</th><th>Invoice</th><th>Customer</th><th>Profit Share</th><th>Paid</th><th>Balance</th></tr></thead>" +
      "<tbody>" + rowsHtml + "</tbody></table>" +
  ...
  return (
    <div className="page">
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20, flexWrap:"wrap", gap:12 }}>
        <div>
          <h2 style={{ fontSize:18, fontWeight:800 }}>🤝 Najib-Orabi Profit Balance</h2>
          <div style={{ fontSize:12, color:T.muted, marginTop:2 }}>100% of net profit from Najib-Orabi location — split 50/50</div>
        </div>
        <div style={{ display:"flex", gap:8 }}>
          <button onClick={printLedger} style={{ background:T.card, border:"1px solid "+T.border, borderRadius:8, padding:"8px 14px", fontSize:12, cursor:"pointer", color:T.text }}>🖨️ Print Ledger</button>
          <button onClick={() => setShowForm(!showForm)} style={{ background:partner.color, border:"none", borderRadius:8, padding:"8px 16px", fontSize:13, fontWeight:700, color:"#fff", cursor:"pointer" }}>+ Record Payment</button>
        </div>
      </div>
      ...
      <div style={{ background:T.card, border:"1px solid "+T.border, borderRadius:12, padding:"14px 20px", marginBottom:20, display:"flex", gap:32, flexWrap:"wrap", alignItems:"center" }}>
        <div>
          <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:4 }}>📈 Total Net Profit</div>
          <div style={{ fontSize:22, fontWeight:800, color:T.green, fontFamily:T.mono }}>{fmt(totalProfit)}</div>
          <div style={{ fontSize:11, color:T.muted }}>{noInvoices.length} sale invoices · Revenue {fmt(totalRevenue)} − COGS {fmt(totalCogs)}</div>
        </div>
        {PARTNERS.map(p => {
          const share = +(totalProfit * p.pct).toFixed(2);
          const paid  = payments.filter(x => x.person === p.key).reduce((s,x) => s+(+x.amount), 0);
          const bal   = share - paid;
          return (
            <div key={p.key} style={{ borderLeft:"3px solid "+p.color, paddingLeft:16 }}>
              <div style={{ fontSize:11, color:T.muted, marginBottom:2 }}>{p.icon} {p.label} (50%)</div>
              <div style={{ fontSize:16, fontWeight:800, color:p.color, fontFamily:T.mono }}>{fmt(share)}</div>
              <div style={{ fontSize:11, color:bal>0?T.red:T.green, marginTop:2 }}>{bal>0?"Owes: "+fmt(bal):"✅ Settled"}</div>
            </div>
          );
      ...
      <div style={{ display:"flex", gap:8, marginBottom:20 }}>
        {PARTNERS.map(p => (
          <button key={p.key} onClick={() => { setTab(p.key); setShowForm(false); }}
            style={{ flex:1, padding:"10px 0", borderRadius:10, border:"2px solid "+(tab===p.key?p.color:T.border),
              background:tab===p.key?p.color+"22":"transparent", color:tab===p.key?p.color:T.muted,
              fontWeight:700, fontSize:14, cursor:"pointer" }}>
            {p.icon} {p.label} <span style={{ fontSize:11, opacity:0.8 }}>(50%)</span>
          </button>
        ))}
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:14, marginBottom:20 }}>
        {[
          { label:"Profit Share",  value:fmt(partnerShare), color:T.yellow, icon:"📈", sub:"50% of net profit" },
          { label:"Total Paid",    value:fmt(totalPaid),    color:T.green,  icon:"✅", sub:partnerPayments.length+" payments" },
          { label:"Balance Owed",  value:fmt(balance),      color:balance>0?T.red:T.green, icon:"💳", sub:balance>0?"Outstanding":"All clear" },
        ].map(c => (
          <div key={c.label} style={{ background:T.card, border:"1px solid "+c.color+"44", borderRadius:12, padding:18 }}>
            <div style={{ fontSize:11, color:T.muted, textTransform:"uppercase", marginBottom:6 }}>{c.icon} {c.label}</div>
            <div style={{ fontSize:22, fontWeight:800, color:c.color, fontFamily:T.mono }}>{c.value}</div>
      ...
      {showForm && (
        <div style={{ background:T.card, border:"2px solid "+partner.color+"44", borderRadius:12, padding:20, marginBottom:20 }}>
          <h3 style={{ fontSize:13, fontWeight:700, marginBottom:14, textTransform:"uppercase", color:partner.color }}>
            Record Payment → {partner.label}
          </h3>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:12, marginBottom:14 }}>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>DATE</div><input type="date" value={form.date} onChange={e=>setForm({...form,date:e.target.value})} /></div>
            <div>
              <div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>AMOUNT</div>
              <input type="number" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})} placeholder="0.00" />
              {balance > 0 && <div style={{ fontSize:10, color:T.muted, marginTop:2 }}>Balance owed: {fmt(balance)}</div>}
            </div>
            <div>
              <div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>METHOD</div>
              <select value={form.payment_method} onChange={e=>setForm({...form,payment_method:e.target.value})}>
                <option value="cash_usd">💵 Cash USD</option>
                <option value="wallet_usdt">💎 USDT</option>
                <option value="bank_transfer">🏦 Bank Transfer</option>
              </select>
            </div>
            <div><div style={{ fontSize:11, color:T.muted, marginBottom:4 }}>NOTES</div><input value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Optional note" /></div>
          </div>
          <div style={{ display:"flex", gap:8 }}>
            <button onClick={save} disabled={saving} style={{ background:partner.color, border:"none", borderRadius:8, padding:"9px 20px", fontSize:13, fontWeight:700, color:"#fff", cursor:"pointer" }}>{saving?"Saving...":"💾 Save Payment"}</button>
            <button onClick={()=>setShowForm(false)} style={{ background:"transparent", border:"1px solid "+T.border, borderRadius:8, padding:"9px 16px", fontSize:13, color:T.muted, cursor:"pointer" }}>Cancel</button>
          </div>
        </div>
      )}
      ...
      <div style={{ background:T.card, border:"1px solid "+T.border, borderRadius:12, overflow:"hidden" }}>
        <div style={{ padding:"12px 20px", borderBottom:"1px solid "+T.border, background:partner.color+"11" }}>
          <span style={{ fontWeight:700, color:partner.color }}>{partner.icon} {partner.label} Ledger</span>
          <span style={{ fontSize:11, color:T.muted, marginLeft:8 }}>(50% of net profit per invoice)</span>
        </div>
        <table>
          <thead>
            <tr>
              <th>Date</th><th>Invoice</th><th>Customer</th>
              <th style={{ color:T.red }}>Profit Share (Owed)</th>
              <th style={{ color:T.green }}>Paid</th>
      ...
      {noInvoices.length > 0 && (
        <div style={{ marginTop:20, background:T.card, border:"1px solid "+T.border, borderRadius:12, overflow:"hidden" }}>
          <div style={{ padding:"14px 20px", borderBottom:"1px solid "+T.border }}>
            <h3 style={{ fontSize:13, fontWeight:700, textTransform:"uppercase", color:T.accent }}>📑 Profit by Invoice</h3>
          </div>
          <table>
            <thead>
              <tr>
                ...
                <th style={{ color:T.green }}>Net Profit</th>
                <th style={{ color:"#a855f7" }}>Najib (50%)</th>
                <th style={{ color:"#00e5ff" }}>Orabi (50%)</th>
              </tr>
            </thead>
            <tbody>
              {noInvoices.map(inv => {
                const p = (+inv.total||0) - (+inv.cogs||0);
                return (
                  <tr key={inv.id}>
                    <td style={{ fontFamily:T.mono, color:T.accent, fontSize:12 }}>{inv.id}</td>
                    <td style={{ fontSize:12, color:T.muted }}>{inv.date}</td>
                    <td style={{ fontWeight:600 }}>{inv.customer}</td>
                    <td style={{ fontFamily:T.mono }}>{fmt(+inv.total||0)}</td>
                    <td style={{ fontFamily:T.mono, color:T.muted }}>{fmt(+inv.cogs||0)}</td>
                    <td style={{ fontFamily:T.mono, fontWeight:700, color:T.green }}>{fmt(p)}</td>
                    <td style={{ fontFamily:T.mono, color:"#a855f7" }}>{fmt(+(p*0.5).toFixed(2))}</td>
                    <td style={{ fontFamily:T.mono, color:"#00e5ff" }}>{fmt(+(p*0.5).toFixed(2))}</td>
                  </tr>
                );
              })}
              <tr style={{ borderTop:"2px solid "+T.border }}>
                <td colSpan={5} style={{ fontWeight:700, textAlign:"right", paddingRight:16 }}>Total:</td>
                <td style={{ fontFamily:T.mono, fontWeight:800, color:T.green }}>{fmt(totalProfit)}</td>
                <td style={{ fontFamily:T.mono, fontWeight:800, color:"#a855f7" }}>{fmt(+(totalProfit*0.5).toFixed(2))}</td>
                <td style={{ fontFamily:T.mono, fontWeight:800, color:"#00e5ff" }}>{fmt(+(totalProfit*0.5).toFixed(2))}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
function ShippingBalance({ suppliers, invoices, onRefresh }) {
  ...
  const ledgerRows = [
    ...shipInvoices.map(inv => {
      // Use shipment_value directly - items may not be loaded
      const shipAmt = inv.shipment_value || 0;
  ...
  const printLedger = () => {
    ...
    const period = fromDate||toDate ? ` (${fromDate||"start"} → ${toDate||"today"})` : " (All Time)";
    const html = `<!DOCTYPE html><html><head><title>Shipping Ledger</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;padding:30px;font-size:12px;}h1{font-size:18px;font-weight:800;margin-bottom:4px;}h2{font-size:14px;color:#666;margin-bottom:20px;}table{width:100%;border-collapse:collapse;}th{background:#000;color:#fff;padding:8px;text-align:left;}td{padding:7px 8px;border-bottom:1px solid #eee;}tr:nth-child(even){background:#f9f9f9;}.footer{margin-top:20px;text-align:center;font-size:10px;color:#999;}</style></head><body><h1>📱 Najib Cell — Shipping Ledger</h1><h2>${supplier?.name||""}${period}</h2><table><thead><tr><th>Date</th><th>Description</th><th>Notes</th><th>Debit</th><th>Credit</th><th>Balance</th></tr></thead><tbody>${rows}</tbody></table><div class="footer">Printed: ${new Date().toLocaleDateString()} • BSM</div><script>window.onload=function(){window.print();window.onafterprint=function(){window.close();}}</script></body></html>`;
    const w = window.open("","_blank","width=900,height=700"); w.document.write(html); w.document.close();
  };
...
function PaymentsPage({ suppliers }) {
  ...
  const printPayment = (p) => {
    const html = `<!DOCTYPE html><html><head><title>Najib Cell — Payment ${p.id}</title><style>
      * { box-sizing: border-box; margin: 0; padding: 0; }
      body { font-family: Arial, sans-serif; color: #000; background: white; padding: 40px; max-width: 400px; margin: 0 auto; }
      h1 { font-size: 20px; font-weight: 800; margin-bottom: 4px; }
      .subtitle { font-size: 13px; color: #666; margin-bottom: 20px; }
    ...
    <div className="page">
      ...
      <div style={{ display:"flex", gap:10, alignItems:"center", flexWrap:"wrap", background:T.card, border:`1px solid ${T.border}`, borderRadius:10, padding:"10px 14px", marginBottom:16 }}>
        ...
        <button onClick={() => {
          ...
          const period = fromDate||toDate ? ` (${fromDate||"start"} → ${toDate||"today"})` : " (All Time)";
          const html = `<!DOCTYPE html><html><head><title>Payments Ledger</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:Arial,sans-serif;padding:30px;font-size:12px;}h1{font-size:18px;font-weight:800;margin-bottom:4px;}h2{font-size:14px;color:#666;margin-bottom:20px;}table{width:100%;border-collapse:collapse;}th{background:#000;color:#fff;padding:8px;text-align:left;}td{padding:7px 8px;border-bottom:1px solid #eee;}tr:nth-child(even){background:#f9f9f9;}.footer{margin-top:20px;text-align:center;font-size:10px;color:#999;}</style></head><body><h1>📱 Najib Cell — Payments Ledger</h1><h2>${period}</h2><table><thead><tr><th>Date</th><th>Supplier</th><th>Notes</th><th>Reference</th><th>Method</th><th>Amount</th><th>Running Total</th></tr></thead><tbody>${rows}</tbody></table><div class="footer">Printed: ${new Date().toLocaleDateString()} • BSM</div><script>window.onload=function(){window.print();window.onafterprint=function(){window.close();}}</script></body></html>`;
          const w = window.open("","_blank","width=900,height=700"); w.document.write(html); w.document.close();
        }} style={{ marginLeft:"auto", background:T.green+"22", border:`1px solid ${T.green}44`, borderRadius:8, padding:"6px 14px", color:T.green, fontSize:12, fontWeight:700, cursor:"pointer" }}>🖨️ Print Ledger</button>
      </div>
...
export default function App() {
  const savedPage = sessionStorage.getItem("BSM_page") || "dashboard";
  ...
  const loadData = useCallback(async () => {
    ...
    const { data: expData } = await supabase.from("expenses").select("*, locations(name)").order("date", { ascending: false });
    setExpenses((expData||[]).map(e => ({ ...e, locationName: e.locations?.name || "" })));
  ...
  const NAV = [
    ...
    { id: "bsm-balance", label: "BSM Balance", icon: "🏦" },
    { id: "najib-orabi-balance", label: "Najib-Orabi Balance", icon: "🤝" },
    { id: "payments", label: "Payments", icon: "🔴" },
    { id: "orders", label: "Sales Orders", icon: "📋" },
    { id: "invoices", label: "Invoices", icon: "🧾" },
    { id: "expenses", label: "Expenses", icon: "💸" },
    { id: "tech", label: "Tech Assistance", icon: "🔧" },
    { id: "accounts", label: "Account Ledger", icon: "⚖️" },
    { id: "capital", label: "Capital & Profit", icon: "💼" },
    { id: "trial-balance", label: "Trial Balance", icon: "📋" },
    ...(isAdmin ? [{ id: "balance-sheet", label: "Balance Sheet", icon: "📊" }] : []),
    ...(isAdmin ? [{ id: "pl", label: "P&L", icon: "📊" }] : []),
        ...
        <nav className="sidebar no-print" style={{ width: sidebarWidth, minWidth: 60, maxWidth: 320, background: T.surface, borderRight: `1px solid ${T.border}`, padding: "28px 0", display: "flex", flexDirection: "column", position: "sticky", top: 0, height: "100vh", flexShrink: 0, transition: "none", overflowY: "auto", overflowX: "hidden" }}>
          <div style={{ padding: "0 16px 28px", borderBottom: `1px solid ${T.border}` }}>
            <div style={{ fontSize: 18, fontWeight: 800, letterSpacing: -0.5, whiteSpace: "nowrap", overflow: "hidden" }}>
              <span style={{ color: T.accent }}>⚡</span> <span className="logo-text">BSM</span>
            </div>
            <div className="logo-text" style={{ fontSize: 10, color: T.muted, marginTop: 4, letterSpacing: 1 }}>BUSINESS MANAGER</div>
          </div>
          <div style={{ padding: "12px 8px", flex: 1 }}>
            {NAV.map(n => (
              <button key={n.id} onClick={() => { sessionStorage.setItem("BSM_page", n.id); setPageHistory([n.id]); }} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "9px 10px", background: page === n.id ? T.accentDim : "transparent", color: page === n.id ? T.accent : T.muted, border: "none", borderRadius: 8, fontSize: 12, fontWeight: page === n.id ? 700 : 400, textAlign: "left", marginBottom: 2, transition: "all .15s", borderLeft: page === n.id ? `2px solid ${T.accent}` : "2px solid transparent", whiteSpace: "nowrap", overflow: "hidden" }}>
        ...
        <main className="main-content" style={{ flex: 1, padding: "28px 32px", overflowY: "auto", overflowX: "auto", minWidth: 0 }}>
          ...
          {loading ? <Loader /> : (
            <>
              ...
              {page === "bsm-balance" && <BSMBalance invoices={invoices} onRefresh={loadData} />}
              {page === "najib-orabi-balance" && <NajibOrabiBalance invoices={invoices} locations={locations} onRefresh={loadData} />}
              ...
              {page === "tech" && <TechPage />}
              {page === "accounts" && <AccountLedger />}
              {page === "capital" && <CapitalPage invoices={invoices} expenses={expenses} locations={locations} />}
              {page === "trial-balance" && <TrialBalance invoices={invoices} expenses={expenses} suppliers={suppliers} clients={clients} />}
              {page === "balance-sheet" && <BalanceSheet invoices={invoices} expenses={expenses} suppliers={suppliers} clients={clients} locations={locations} />}
              {page === "pl" && <ProfitLoss invoices={invoices} locations={locations} userProfile={userProfile} />}
              {page === "reports" && <Reports invoices={invoices} products={products} locations={locations} setPage={setPage} expenses={expenses} />}
